...
To customize Visual Studio for Mac, you can create an extension package that builds off extension points contained in add-in hosts within pre-existing libraries in Visual Studio for Mac, as illustrated by the diagram below:
...
These--} Visual Studio for Mac Core and IDE assemblies are {-- now--}{>> Don't think we need the 'earlier versions' discussion, since this is VS, not XS <<} automatically referenced at build time. 

...
1. In Solution Window, select the solution (the top node).
...
6. Select **OK**. The new Solution Run Configuration is set as the active run configuration:
   ...
   You can see that two projects are configured to start because both projects are **bold** in the Solution Window. In the toolbar, the new run configuration is set as the current Solution Run Configuration.

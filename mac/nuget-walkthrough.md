...
Use the search box in the top-right corner to find a specific package, for example `azure`. When you've found a package you wish to use, select it and click the **Add Package** button to begin installation.
...
*   The **Packages** node displays each NuGet package you have downloaded. You can update or remove a package from this list.
...
Ensure you add and required `using` directives to the top of your file:
...
Package updates can be done either all at once, view the **Packages** node, or individually on each component.

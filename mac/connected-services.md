...
4. Choose **Accept** on the **License Acceptance** dialogs for the NuGet packages. There might be two dialogs to accept, one for the MobileClient and dependencies, and another for SQLiteStore, which is required for offline data sync:
...
6. Select an app service from the list. This will fill the template code for the `MobileServiceClient` object with the corresponding URL of the app service on Azure:
    ...
    [![Select an app service from the list](media/connected-services-image006-sml.png "Select an app service from the list")](media/connected-services-image006.png#lightbox)
...
9. If you don't have any existing app service, you can create new services from within Visual Studio for Mac. Click the **New** button in the bottom left of the services list to open the **New App Service** dialog:

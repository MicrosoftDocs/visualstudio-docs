...
|`AssembliesGeneratedDuringBuild`|Optional **String[]** parameter.<br /><br /> Specifies references to assemblies that change during the build process. For example, a [!INCLUDE[TLA#tla_visualstu2005](../msbuild/includes/tlasharptla_visualstu2005_md.md)] solution may contain one project that references the compiled output of another project. In this case, the compiled output of the second project can be added to **AssembliesGeneratedDuringBuild**.<br /><br /> Note: **AssembliesGeneratedDuringBuild** must contain references to the complete set of assemblies that are generated by a build solution.|  

...
|    DreamSpark/ADT4T Retail Key  | Retail keys for DreamSpark and Azure Dev Tools for Teaching allow activation on one device. These keys are issued in batches and are primarily intended for student consumption. |
|    DreamSpark/ADT4T Lab Key | Lab use keys for DreamSpark programs that allow multiple activations. DreamSpark and Azure Dev Tools for Teaching Lab Keys are intended for use in university computer lab scenarios. |
|    DreamSpark/ADT4T MAK Key | MAK keys for DreamSpark and Azure Dev Tools for Teaching program customers. |
|

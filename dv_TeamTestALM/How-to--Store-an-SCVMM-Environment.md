...
1.  In the **Lab** tab of Test Manager, make sure that the environment is stopped.  
...
1.  In the **Library** tab of Test Manager, choose **Environments**.  
...
1.  In the In the **Library** tab of Test Manager, make sure that the environment is shut down.  
  
2.  Choose **Store to library** on the toolbar.  
  
 When you deploy an environment that uses network isolation, Lab Management adds an additional network adapter to each virtual machine that presents a unique alias identity to the external domain. Therefore, you can deploy multiple copies of the stored environment at the same time without encountering computer name conflicts.  
...
2.  In the **Lab** tab of Test Manager, make sure that the environment is running.  
...
5.  Exit Environment Viewer and then shut down the environment from the Test Manager **Lab** tab.  

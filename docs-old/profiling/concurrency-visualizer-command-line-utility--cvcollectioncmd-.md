...
|------------|-----------------|----------------|-------------------|  
|Query|Returns whether collection can be started.|None|0 if collection is ready to start.<br /><br /> 1  if collection is already in progress.<br /><br /> 2 if collection isn’t in progress, but one or more of the required [ETW](../Topic/ETW%20Tracing.md) sessions is already enabled.|  
...
|MajorVersion|Specifies the major version of the config file.|Must be 1 for [!INCLUDE[vs_dev11_long](../codequality/includes/vs_dev11_long_md.md)] projects. If not 1, the utility will not work.|  
|MinorVersion|Specifies the minor version of the config file.|Must be 0 for [!INCLUDE[vs_dev11_long](../codequality/includes/vs_dev11_long_md.md)] projects. If not 0, the utility will not work.|  

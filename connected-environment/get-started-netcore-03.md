...
Run this command (in a new command or terminal window) to create an ASP.NET Core Web App in a folder named `webfrontend`.
```
...
In the [previous section](get-started-netcore-02.md), we kicked off the `vsce env create` command to create a development environment in Azure. Check back that it was successfully created. You can always check the environments you have access to, and their status, with this command: `vsce env list`. 
...
Keep an eye on the command's output as it progresses, you'll notice several things:

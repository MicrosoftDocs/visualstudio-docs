...
> [!NOTE]
>  The console apps in these examples use <xref:System.Threading.Tasks.Task.Wait?qualifyHint=False> method to prevent the application from terminating in `Main`. You shouldn’t use the <xref:System.Threading.Tasks.Task.Wait?qualifyHint=False> method outside of console applications  because a deadlock situation can occur.  

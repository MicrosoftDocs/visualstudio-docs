...
An expression uses the `=` with one or both operands of the [Object Data Type](../Topic/Object%20Data%20Type.md).  
 ...
 When a variable or expression evaluates to `Object`, the compiler must perform *late binding*, which causes extra operations at run time. It also exposes your application to potential run-time errors. For example, if you assign a <xref:System.Windows.Forms.Form?qualifyHint=False> to an `Object` variable and then try to use it with the `=` operator, the runtime throws an <xref:System.InvalidCastException?qualifyHint=False> because Visual Basic cannot convert a <xref:System.Windows.Forms.Form?qualifyHint=False> object to a data type suitable for value comparison. Even if both operands evaluate to type <xref:System.Windows.Forms.Form?qualifyHint=False>, the operation fails because `=` is not defined for <xref:System.Windows.Forms.Form?qualifyHint=False> operands.  

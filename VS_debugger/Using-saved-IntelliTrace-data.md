...
-   Matching source files and symbol (.pdb) files for your application code. Otherwise, Visual Studio can't resolve the source locations and shows the message "Symbols not found." See [Specify Symbol (.pdb) and Source Files](../VS_debugger/Specify-Symbol--.pdb--and-Source-Files-in-the-Visual-Studio-Debugger.md) and [Diagnose problems after deployment](../VS_debugger/Diagnose-problems-after-deployment.md).  
...
-   An .iTrace file from one of these sources:  
    ...
    |**Source**|**See**|  
    |----------------|-------------|  
    |An IntelliTrace session in Visual Studio Enterprise (but not Professional or Community editions)|[IntelliTrace Features](../VS_debugger/IntelliTrace-Features.md)|  
    |A test session in Microsoft Test Manager. This attaches an .iTrace file to a Team Foundation Server work item.|[Collect more diagnostic data in manual tests](../dv_TeamTestALM/Collect-more-diagnostic-data-in-manual-tests.md)|  
    |Microsoft Monitoring Agent, either alone or with System Center 2012 R2 Operations Manager, for ASP.NET web apps and SharePoint applications running in deployment|-   [Diagnose problems after deployment](../VS_debugger/Diagnose-problems-after-deployment.md)<br />-   [What's New for System Center 2012 R2 Operations Manager](http://technet.microsoft.com/library/dn249700.aspx)|  
...
3.  Expand that call to review any nested calls and parameter values that were recorded at that point in time.  
     ...
     Now you can review other recorded values, the call stack, step through your code, or use the **IntelliTrace** window to [move backwards or forwards "in time" between other methods](../VS_debugger/IntelliTrace.md) that were called during this performance event.  
...
1.  Under **Exception Data**, review the recorded exception events, their types, messages, and when the exceptions happened. To dig deeper into the code, start debugging from the most recent event in a group of exceptions.  
     ...
     Now you can review other recorded values, the call stack, or use the **IntelliTrace** window to [move backwards or forwards "in time" between other recorded events](../VS_debugger/IntelliTrace.md), related code, and the values recorded at those points in time.  
...
1.  Copy the SharePoint correlation ID from its source.  
     ...
     ![IntelliTrace &#45; SharePoint error &#45; correlation ID](../VS_debugger/media/SharePointError_IntelliTrace.png "SharePointError_IntelliTrace")  
...
2.  Open the .iTrace file, then go to **Analysis** and enter the SharePoint correlation ID to review the matching web request and recorded events.  
     ...
     ![IntelliTrace log &#45; Enter SharePoint correlation ID](../VS_debugger/media/EnterSharePointCorrelationID.png "EnterSharePointCorrelationID")  
...
3.  Under **Request Events**, examine the events. Starting from the top, events appear in the order that they happened.  
    ...
    2.  Choose **Start Debugging** to start debugging at the point where the event happened.  
     ...
     ![IntelliTrace log file &#45; View web request &#43; events](../VS_debugger/media/EnterSharePointCorrelationID2.png "EnterSharePointCorrelationID2")  
...
3.  Choose **Debug Exception** to start debugging at the point where and when the exception happened.  
     ...
     ![IntelliTrace log &#45; SharePoint unhandled exceptions](../VS_debugger/media/SharePointUnhandledExceptions_IntelliTrace.png "SharePointUnhandledExceptions_IntelliTrace")  
  
 For a walkthrough, see [Walkthrough: Debugging a SharePoint Application by Using IntelliTrace](../Topic/Walkthrough:%20Debugging%20a%20SharePoint%20Application%20by%20Using%20IntelliTrace.md). For the kinds of data that the agent records, see [IntelliTrace Features](../VS_debugger/IntelliTrace-Features.md).  
...
2.  At the bottom of **Threads List**, choose **Start Debugging**. You can also double-click a thread.  
     ...
     To start debugging from where the app begins, double-click **Main Thread**. See [IntelliTrace Features](../VS_debugger/IntelliTrace-Features.md).  

...
|Code window|<xref:Microsoft.VisualStudio.TextManager.Interop.VsCodeWindow>|A document window that contains one or more text views. When [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] is in multiple-document interface (MDI) mode, the code window is an MDI child.|  

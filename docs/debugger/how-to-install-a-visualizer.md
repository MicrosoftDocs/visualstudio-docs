...
1. Locate the DLL that contains the visualizer you built.
   ...
   Typically, it is best if both the debugger-side DLL and the debuggee-side DLL are compiled for **Any CPU**. The debugger-side DLL must be either **Any CPU** or **32-bit**. The target platform for the debuggee-side DLL should correspond to the debugee process.

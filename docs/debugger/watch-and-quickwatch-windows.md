...
While you're debugging, you can use the **Watch** and **QuickWatch** windows to watch variables and expressions.  The difference is that the **Watch** window can display several variables, while the **QuickWatch** window displays a single variable at a time.

The windows are only available during a debugging session. To open the **Watch** window, choose **Debug** > **Windows** > **Watch** > **Watch (1, 2, 3, 4)**). To open the **QuickWatch** window, either right-click the variable and choose **QuickWatch** or choose **Debug** > **QuickWatch**.
...
3. Open the **QuickWatch** window (right-click `a`, then choose **QuickWatch**, or select `a` and press **SHIFT+F9**).
...
You can observe more than one variable with the **Watch** window. For example, if you have the following code:
...
For example, if you have the code listed in the preceding section, you can get the average of the three values by using this expression:

![Watch Expression](../debugger/media/watchexpression.png "WatchExpression")

The rules for evaluating expressions in the **Watch** window are generally the same as the rules for evaluating expressions in your coding language. If your expression has a syntax error, you can expect the same compiler error that you would see in the code editor. Here's an example:
...
A refresh icon (a circular arrow) might appear when an expression is evaluated in the **Watch** window. If you have property evaluation turned off (**Tools** > **Options** > **Debugging** > **Enable property evaluation and other implicit function calls**), and you have the following code:
...
If you set a watch on the `Count` property of the list, you should see something like the following image:

![RefreshWatch](../debugger/media/refreshwatch.png "RefreshWatch")

The preceding illustration shows an error or a value that is out of date. You can generally refresh the value by clicking the icon, but in some cases you might prefer not to refresh it. First you need to know why the value wasn't evaluated.

A tooltip provides information about why the expression wasn't evaluated if you point to the icon. If the circling arrows appear, the expression wasn't evaluated for one of the following reasons:
...
- You've turned off automatic evaluation of properties and implicit functions calls by the debugger (**Tools** > **Options** > **Debugging** > **Enable property evaluation and other implicit function calls**). The expression can't be automatically evaluated then.

To refresh the value, click the refresh icon or press the spacebar. The debugger tries to reevaluate the expression. The refresh icon may appear because automatic evaluation of properties and other implicit function calls was turned off. In this case, the debugger can evaluate the expression.

An icon may appear that is a circle with two wavy lines that resemble threads. This icon means the debugger doesn't evaluate the expression because of a potential cross-thread dependency. In other words, evaluating the code requires other threads in your application to run temporarily. When you are in break mode, all threads in your application are typically stopped. Allowing other threads to run temporarily can have unexpected effects on the state of your program and causes the debugger to ignore events such as breakpoints and exceptions thrown on those threads.
...
An expression known to have side effects is evaluated only once, when you first enter it. Further evaluations are disabled. You can manually override this behavior by clicking the update icon that appears next to the value.
...
> [!NOTE]
> Object IDs create weak references, and don't prevent the object from being garbage collected. They are valid only for the current debugging session.
...
3. When execution stops in the breakpoint, find the variable in the **Locals** window, right-click it, and select **Make Object ID**.

   You should see a **$** plus a number in the **Locals** window, which is the object ID.
...
6. Continue debugging. When execution stops in the `DoSomething()` method, the **Watch** window displays the `Person` object.
...
The **Watch** window may display an object created from a type that implements the <xref:System.Dynamic.IDynamicMetaObjectProvider>. When this object is displayed, the debugger adds a special **Dynamic View** node to the **Autos** display. This node shows the dynamic members of the dynamic object but doesn't allow editing of the member values.
...
Evaluating the members of a **Dynamic View** can have side effects. For an explanation of what side effects are, see [Side Effects and Expressions](#bkmk_sideEffects). For C#, the debugger doesn't automatically reevaluate the values shown in the **Dynamic View** when you step to a new line of code. For Visual Basic, expressions added through the **Dynamic View** are automatically refreshed.
...
The **Dynamic View** also enhances the debugging experience for COM objects. When the debugger gets to a COM object wrapped in **System.__ComObject**, it adds a **Dynamic View** node for the object.

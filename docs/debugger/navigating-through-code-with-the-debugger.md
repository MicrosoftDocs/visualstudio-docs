...
While you are debugging, a yellow highlight shows the code line that will execute next.  
...
To run to the cursor location, in source code or the **Call Stack** window, select the line you want to break at, right-click and select **Run To Cursor**, or press **Ctrl**+**F10**.. This is like setting a temporary breakpoint.

### Run to Click 

While paused in the debugger, hover over a statement in source code or the **Disassembly** window, and select the **Run execution to here** green arrow icon. This eliminates the need to set a temporary breakpoint.
...
While the debugger is paused, a yellow arrowhead in the margin of the source code or **Disassembly** window marks the location of the next statement to be executed. You can change the next statement to execute by moving this arrowhead. You can skip over a portion of code, or return to a previous line. You can use this for situations such as skipping a section of code that contains a known bug.  
...
The program counter jumps directly to the new location, and instructions between the old and new execution points are not executed. However, if you move the execution point backwards, the intervening instructions are not undone.  
...
By default, the debugger tries to debug only your app code by enabling a setting called *Just My Code*. See [Just My Code](../debugger/just-my-code.md) for more details about how this feature works for different project types and languages, and how you can customize it. 
...
1. While you are debugging, open the **Modules** window by selecting **Debug** > **Windows** > **Modules**, or pressing **Ctrl**+**Alt**+**U**.  
...
##  <a name="BKMK_Step_into_properties_and_operators_in_managed_code"></a> Step into properties and operators in managed code  
 By default, the debugger steps over properties and operators in managed code. This usually provides a better debugging experience. To enable stepping into properties and operators, in **Tools** (or **Debug**) > **Options** > **Debugging** > **General**, clear the **Step over properties and operators (Managed only)** check box.
...
- *"Class **X** has a custom debugger view that blocks using data breakpoints on data specific only to it."*
  ...
  - Data breakpoints are only supported on memory that is not generated by the debugger. The memory has been flagged as possibly being owned by an object generated by a DebuggerTypeProxy attribute or something else that is not a part of the debugged application.

...
- *"Class **X** has a custom debugger view that blocks using data breakpoints on data specific only to it."*
  ...
  - Data breakpoints are only supported on memory that is not generated by the debugger. The memory has been flagged as possibly being a part of a DebuggerTypeProxy attribute or something else.
  - **Workaround**: You can set the data breakpoint by expanding the "Raw View" of the object instead of looking at the DebuggerTypeProxy's fields.

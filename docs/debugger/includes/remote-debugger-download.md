...
>[!NOTE]
>-To debug Windows 10 apps on ARM devices, use the ARM64 download available with the latest version of the remote tools.  
>-To debug on Windows RT devices, use the ARM version, which is only available in the Visual Studio 2015 RTW download.

...
To debug XAML code behind, such as data binding code, from the XAML designer, you can use **Debug** > **Attach to Process**.
  
## Debug at design time with the Immediate window  

You can use the Visual Studio **Immediate** window to execute a function or subroutine when your app is not running. If the function or subroutine contains a breakpoint, Visual Studio will break at the breakpoint. You can then use the debugger windows to examine your program state. This feature is called *debugging at design time*.  

The following example is in Visual Basic. You can also use the **Immediate** window at design time in C#, C++, and F# apps.

1. Paste the following code into a new Visual Basic console app:  
...
1. Select **Continue** on the Visual Studio toolbar. The app ends, and **1** returns in the **Immediate** window. Make sure you are still in design mode.  
...
1. Select **Continue** again. The app ends, and **2** returns in the **Immediate** window. Make sure that you are still in design mode.
   
1. Close the console window to end the session. Right-click in the **Immediate** window and select **Clear All** to clear its contents. 
...
1. In a Visual Studio project, add a new XAML page, such as *temp.xaml*. Leave the new XAML page empty. 
...
1. In the **Attach to Process** dialog box, set the **Attach to** field to the correct code type for your .NET version, such as **Managed Code (CoreCLR)**. 

1. Select the correct designer process from the **Available Processes** list, and select **Attach**.

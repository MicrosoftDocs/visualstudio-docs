...
> [!Important]
> Microsoft has deprecated the Python extensions for App Service as described in this article in favor of a direct deployment to App Service on Linux. The extensions still continue to work but will not be serviced, nor will extensions be made for newer versions of Python. To deploy to App Service on Linux, see [Create a Python web app in Azure App Service on Linux](/azure/app-service/containers/quickstart-python).

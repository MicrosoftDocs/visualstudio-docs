...
Follow this procedure to create the solution and Flask web project:
...
1. In Visual Studio, select **File** > **New** > **Project** and search for "Flask." Then, select the **Blank Flask Web Project** template. (The template is also found under **Python** > **Web** in the list on the left.)
...
## Examine Git controls and publish to remote repository

::: moniker range="vs-2022"

<!-- Determine if Team Explorer is still appropriate to reference for VS 2022 -->

In the next procedure, you familiarize yourself with Visual Studio's Git controls and the **Team Explorer** window where you work with source control features.
...
1. After you create a repository, a set of Git controls appears at the bottom right in the Visual Studio main window:
   ...
   From left to right, the Git controls show the number of outgoing and incoming commits, the number of uncommitted changes, the current branch, and the current repository. Git controls are also available on the **Git** menu on the main toolbar.

1. Select **View** > **Git changes** to open the Visual Studio **Git Changes** window. Because the newly created project is already committed to source control automatically, you don't see any pending changes:
...
<!-- UI has changed -- Locate commands in VS 2022 ----
...
1. On the Visual Studio status bar, select the unpushed commits button (the up arrow with **2**) to open the **Synchronization** page in **Team Explorer**. Because you have only a local repository, the page provides easy options to publish the repository to different remote repositories.
   ...
   :::image type="content" source="media/flask/step-01-team-explorer.png" alt-text="Screenshot of the Team Explorer window showing the available Git repository options for source control." border="false" lightbox="media/flask/step-01-team-explorer.png":::
...
1. When selecting any of the **Publish** controls, **Team Explorer** prompts you for more information. For example, when publishing the sample for this tutorial, the repository itself had to be created first. In this case, the **Push to Remote Repository** option was used with the repository's URL.
   ...
   :::image type="content" source="media/flask/step-01-push-to-github.png" alt-text="Screenshot that shows how to work in the Team Explorer window to push to an existing remote repository." border="false" lightbox="media/flask/step-01-push-to-github.png":::
...
> [!TIP]
> To quickly navigate within **Team Explorer**, select the header (that reads **Changes** or **Push** in the images above) to see a popup menu of the available pages.
...
### Prevent Visual Studio from auto-committing new project

You can prevent Visual Studio from auto-committing a new project. To disable auto-commit, go to the **Settings** page in **Team Explorer**, select **Git** > **Global settings**, clear the option labeled **Commit changes after merge by default**, then select **Update**.

## Create virtual environment and exclude from source control
...
<!-- Confirm Team Explorer is still appropriate for these actions in VS 2022 -->

1. In **Solution Explorer**, right-click the **Python Environments** node and select **Add Environment**.

   :::image type="content" source="media/flask/step-01-add-virtual-environment-command.png" alt-text="Screenshot that shows the Add Virtual environment command in Solution Explorer." border="false" lightbox="media/flask/step-01-add-virtual-environment-command.png":::
...
1. Select **Create** to accept the defaults, in the Add Virtual Environment dialog. (You can change the name of the virtual environment if you want, which just changes the name of its subfolder, but `env` is a standard convention.)
   ...
   :::image type="content" source="media/tutorials-common/step-01-add-virtual-environment-found-requirements.png" alt-text="Screenshot that shows the Add virtual environment dialog with requirements.txt message." border="false" lightbox="media/tutorials-common/step-01-add-virtual-environment-found-requirements.png"::: 
...
1. On the Visual Studio Git controls (on the status bar), select the changes indicator (that shows **99\***) which opens the **Changes** page in **Team Explorer**.
   ...
   Creation of the virtual environment brings in thousands of changes, but you don't need to include them in source control. You (or anyone else cloning the project) can always recreate the environment from the _requirements.txt_ file.

   To exclude the virtual environment, right-click the **env** folder and select **Ignore these local items**.

   :::image type="content" source="media/flask/step-01-ignore-local-items.png" alt-text="Screenshot that shows how to ignore a virtual environment in source control changes." border="false" lightbox="media/flask/step-01-ignore-local-items.png"::: 

1. After you exclude the virtual environment, the only remaining changes are to the project file and the _.gitignore_ file. This file contains an added entry for the virtual environment folder. You can double-click the file to see a diff.

1. Enter a commit message, select **Commit All**, then push the commits to your remote repository.
...
1. An **Add Virtual Environment** dialog appears, with a message saying **We found a requirements.txt file.** This message indicates that Visual Studio uses that file to configure the virtual environment.
   ...
   :::image type="content" source="media/tutorials-common/step01-add-virtual-environment-found-requirements.png" alt-text="Screenshot of the Add virtual environment dialog with requirements.txt message in Visual Studio." border="false" lightbox="media/tutorials-common/step01-add-virtual-environment-found-requirements.png"::: 
...
1. Consent to administrator privileges (if prompted) and wait while Visual Studio downloads and installs packages. For Flask and its dependencies, the process can require expanding about 1,000 files in over 100 subfolders. You can monitor the progress in the Visual Studio **Output** window. You can see a description of Flask's dependencies on the [Flask installation](https://flask.palletsprojects.com/installation/#installation) page (flask.pcocoo.org).

1. On the Visual Studio Git controls (on the status bar), select the changes indicator (that shows **99\***) which opens the **Changes** page in **Team Explorer**.

   Creation of the virtual environment brings in hundreds of changes, but you don't need to include them in source control. You (or anyone else cloning the project) can always recreate the environment from the _requirements.txt_ file.
...
A virtual environment is a great way to isolate your app's exact dependencies. Such isolation avoids conflicts within a global Python environment, and aids both testing and collaboration. Over time, as you develop an app, you invariably bring in many helpful Python packages. By keeping packages in a project-specific virtual environment, you can easily update the project's _requirements.txt_ file that describes that environment, which is included in source control. When you copy the project other computers, including build servers, deployment servers, and other development computers, it's easy to recreate the environment. You can recreate the environment by using only the _requirements.txt_ file, which is why the environment doesn't need to be in source control. For more information, see [Use virtual environments](selecting-a-python-environment-for-a-project.md#use-virtual-environments).

### Remove virtual environment under source control

You can remove a virtual environment already committed to source control. First, edit your _.gitignore_ file to exclude the folder: find the section at the end with the comment `# Python Tools for Visual Studio (PTVS)` and add a new line for the virtual environment folder, such as `/BasicProject/env`. (Because Visual Studio doesn't show the file in **Solution Explorer**, open it directly using the **File** > **Open** > **File** menu command. You can also open the file from **Team Explorer**: on the **Settings** page, select **Repository Settings**, go to the **Ignore & Attributes Files** section, then select the **Edit** link next to **.gitignore**.)

Second, open a command window, navigate to the folder like _BasicProject_ that contains the virtual environment folder such as _env_, and run `git rm -r env`. Then commit those changes from the command line (`git commit -m 'Remove venv'`) or commit from the **Changes** page of **Team Explorer**.

## Examine the boilerplate code

In this procedure, you examine the boilerplate code in the Project file (_.py_) that Visual Studio creates based on your template selection.

1. After project creation completes, you see the solution and project in **Solution Explorer**, where the project contains only two files, _app.py_ and _requirements.txt_:

   :::image type="content" source="media/flask/step01-blank-flask-project-in-solution-explorer.png" alt-text="Screenshot that shows Blank Flask project files in Solution Explorer." border="false" lightbox="media/flask/step01-blank-flask-project-in-solution-explorer.png"::: 

1. As noted earlier, the _requirements.txt_ file specifies the Flask package dependency. The presence of this file is what invites you to create a virtual environment when first creating the project.

1. The single _app.py_ file contains three parts. The first part is an `import` statement for Flask. This statement creates an instance of the `Flask` class, which is assigned to the variable `app`. This part also assigns a `wsgi_app` variable (which is useful when you deploy to a web host, but not used for now):
...
1. The second part is at the end of the file. It's a section of optional code that starts the Flask development server with specific host and port values taken from environment variables (defaulting to `localhost:5555`):
...
The `name` argument in a Flask class is the name of the app's module or package, and tells Flask where to look for templates, static files, and other resources that belong to the app. For apps contained in a single module, `__name__` is always the proper value. It's also important for extensions that need debugging information. For more information, and other arguments, see the [Flask class documentation](https://flask.palletsprojects.com/api/#flask.Flask) (flask.pocoo.org).
...
Flask can work with variable URL routes and query parameters. In a route, you mark any variable with `<variable_name>`, and Flask passes the variable to the function using a named argument in the URL path. For example, a route in the form of `/hello/<name>` generates a string argument called `name` to the function. Query parameters are available through the `request.args` property, specifically through the `request.args.get` method.
...
1. In Visual Studio, select **Debug** > **Start Debugging** (**F5**) or use the **Web Server** button on the toolbar (the browser you see might vary):
   ...
   :::image type="content" source="media/tutorials-common/run-web-server-toolbar-button.png" alt-text="Screenshot that shows the Run web server toolbar button in Visual Studio." border="false" lightbox="media/tutorials-common/run-web-server-toolbar-button.png":::
...
1. When the server starts, you see a console window opens that displays the server log. Visual Studio then automatically opens a browser to `http://localhost:<port>`, where you should see the message rendered by the `hello` function:
   ...
   :::image type="content" source="media/flask/step01-first-run-success.png" alt-text="Screenshot that shows the Flask project default view." border="false" lightbox="media/flask/step01-first-run-success.png"::: 
...
There's a difference between using the **Debug** menu commands and the server commands on the project's **Python** submenu.

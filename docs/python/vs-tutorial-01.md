...
Visual Studio provides first-class language support for Python. This quickstart guides you through the following steps:

- [Prerequisites](#prerequisites)
- Creating a new empty Python application
- Writing code to see Visual Studio IntelliSense at work
- Create more code in the interactive REPL window
- Run the completed program by itself
- Run the completed program in the Visual Studio debugger
...
1. Another feature is the ability to quickly send multiple lines of code from an editor window to the interactive window. This action helps you work with that code fragment in the rapid REPL environment rather than having to run the whole program. To see this feature, first replace the `for` loop in your `hello.py` file with the following:
...
1. Select all the code in hello.py (including the `import` statements), right-click, and select **Send to Interactive** (or press Ctrl+Enter). The code is immediately pasted into the interactive window and run. Because the code defines a function, you can quickly test that function by calling it a few times:

    ![Sending code to the interactive window and testing it](../images/vs-getting-started-python-14-interactive4.png)

1. Using Ctrl+Enter with the caret on a single line in the editor is also an easy way to step through your code. Ctrl+Enter runs the current line in the interactive window, then automatically set the caret to the next line. By pressing Ctrl+Enter repeatedly, then, you can run all the code in a file step by step.

1. You can also copy and paste multiple lines of code from any source, such as the snippet below, into the interactive window. When pasted, the interactive window runs that code as if you'd typed it in:
...
1. The code works, but the output isn't very inspiring; using a different step value in the `for` loop would show more of the cosine wave. Fortunately, because the entire `for` loop is in the REPL history as a single unit, it's easy to go back and make whatever changes you want and then test the function again. Press the up arrow to recalls the `for` loop. Press the left arrow to start navigating in the code, then use the up and right arrows to change the `range` specification to `range(0, 360, 12)`. Then press Ctrl+Enter to run the whole statement again:
...
1. When you're satisfied with code you're written in the interactive window, select it, right-click and select **Copy Code** (Ctrl+Shift+C), and then paste into the editor. Notice how the command automatically omits any output as well as the `>>>` and `...` prompt text. For example, the image below shows using the **Copy Code** command on a selection that includes prompts and output:

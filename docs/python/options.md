...
| Wait for input when process exits abnormally<br/><br/>Wait for input when process exits normally | On<br/><br/>On | A Python program started from Visual Studio runs in its own console window. By default, the window will wait for you to press a key before closing it. To remove that prompt and close the window automatically, clear either or both of these options. |

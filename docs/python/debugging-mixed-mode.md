...
For a ten minute introduction to building, testing, and debugging native C modules with Visual Studio, see the following video.

[![Deep Dive: Creating Native Modules](media/video-thumbnails/NativeModules.png)](https://youtu.be/D9RlT06a1EI)
...
The "[C++ View]" node provides a representation of the underlying C/C++ structure for a value, idental to what you'd see in a native frame. For example, it shows an instance of `_longobject` (for which `PyLongObject` is a typedef) for a Python long integer, and it will try to infer types for native classes that you have authored yourself. The children of this node are editable.

...
1. In _Python_CustomCommands.py_, add the code `print("Hello custom commands")`.
...
1. Select the **Run startup file** command and you should see a command window appear with the text **Hello custom commands** followed by **Press any key to continue**. Press a key to close the window.
   ...
   > [!Note]
   > The activated environment for your project is the environment the script will run in.
...
You're likely attempting to run a console command with `ExecuteIn="output"`, in which case Visual Studio might crash trying to parse the output. Use `ExecuteIn="console"` instead. (See [Issue 3682](https://github.com/Microsoft/PTVS/issues/3681).)

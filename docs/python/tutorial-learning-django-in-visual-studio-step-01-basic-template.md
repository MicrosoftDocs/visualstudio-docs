---
title: Learning Django in Visual Studio, step 1
...
# Getting started with the Django web framework in Visual Studio, step 1
...
| Visual Studio project template | Tutorial steps | Description |
| --- | --- | --- |
| Blank Django Web Project | TODO | Minimal boilerplate code for a Django project with no Django apps. The present tutorial starts with this template and builds up an simple app to demonstrate Django fundamentals such as project, apps, views, templates, and static files. You also create a Git repository and a virtual environment from the very beginning. |
| Django Web Project | TODO | A full-featured Django project with an app containing three sample pages and a SQLite database. The template uses the Twitter Bootstrap framework for responsive web design. This section of the tutorial explores more about views and templates along with URL routing and authentication, which are features included in the template.  |
| Polls Django Web Project | TODO | A Django project that implements a polling app. This section of the tutorial covers using the database, models, migrations, and extending the administrative interface. |
...
## Create a Visual project and solution
...
1. In Visual Studio, select **File > New > Project**, search for "Django", and select the **Blank Django Web Project** template. (The template is also found under **Python > Web** in the left-hand list.)
    ...
    ![New project dialog in Visual Studio for the Blank Django Web Project](media/django/section01-new-blank-project.png)
...
1. After a moment, Visual Studio prompts you with a dialog saying "This project requires external packages" (shown below) because the template includes a `requirements.txt` file referencing the latest Django 1.x package. (Select **Show required packages** to see the exact dependencies.) Select the option, **Install into a virtual environment**.
    ...
    ![Prompt saying This project requires external packages](media/django/section01-requirements-prompt.png)
...
## Examine the Git controls and publish to a remote repository
...
![Git controls in the Visual Studio window](media/django/section01-git-controls.png)
...
![Team Explorer window showing available Git repository options for source control](media/django/section01-team-explorer.png)
...
![Team Explorer window for pushing to an existing remote repository](media/django/section01-push-to-github.png)
...
1. In your project root is `manage.py`, the Django command-line administrative utility that Visual Studio automatically sets as the project startup file. You run the utility on the command line using `python manage.py <command> [options]`, or use just `manage.py <command> [options]` from the Python interactive window in Visual Studio. You can also use `django-admin <command> [options]` or `python -m django <command> [options]`.
    ...
    ![Django commands on a Python project context menu](media/django/section01-django-commands-menu.png)

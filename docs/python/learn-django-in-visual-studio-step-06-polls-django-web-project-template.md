...
- Set up a continuous integration/continuous deployment pipeline on a service like Azure DevOps Services. In addition to working with source control (on Azure DevOps Services, GitHub, or elsewhere), you can have Azure DevOps Services automatically run your unit tests as a pre-requisite for release, and also configure the pipeline to deploy to a staging server for additional tests before deploying to production. Azure DevOps Services, furthermore, integrates with monitoring solutions like App Insights and closes the whole cycle with agile planning tools. For more information, see:
  ...
  - [Python development in Azure with Azure DevOps Services (video, 11m 21s)](https://azure.microsoft.com/resources/videos/connect-2017-python-development-in-azure-with-visual-studio-team-services/).

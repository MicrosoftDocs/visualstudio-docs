...
After completing the Connected Services process, you can create a managed identity user for your app to use and set permissions by executing the following query in the database. To execute the following statements, use the [query editor](/azure/azure-sql/database/connect-query-portal?view=azuresql) in the Azure portal.

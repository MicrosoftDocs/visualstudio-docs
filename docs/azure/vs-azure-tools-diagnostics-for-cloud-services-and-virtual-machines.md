...
> [!WARNING]
> In Visual Studio 2022, the deprecated Azure Storage Emulator was replaced with the [Azurite emulator](/azure/storage/common/storage-use-azurite) when debugging your Cloud Service projects. This Azurite emulator does not work with the diagnostics plugin due to lack of support for legacy APIs. If the local diagnostics plugin is needed for testing,  either update the connection string in the local service configuration (.cscfg) to an Azure storage account or download the previous [Azure storage emulator](/azure/storage/common/storage-use-emulator).
 > In the Visual Studio 17.10 release, the local Azure Diagnostics plugin is being deprecated and will be disabled by default with the option to enable in that release. Due to the deprecation, the plugin will likely be removed in a future release of Visual Studio. This plugin is only used with the local Azure Compute emulator and disabling doesn't impact Cloud Services that are deployed and using the Azure Diagnostics extension in Azure.

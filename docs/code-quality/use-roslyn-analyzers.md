...
Analyzers run on all source files in a project and reported violations on them. However, these violations are not useful on generated code files, such as say designer generated code files, temporary source files generated by build system, etc. Users cannot manually edit these files and/or are not concerned about fixing violations in such tooling generated files.
...
2. Add `generated_code = true | false` entry for specific files and/or folders. For example, to treat all files whose name ends with `.MyGenerated.cs` as generated code, the entry would be as follows:

...
Starting in .NET 5.0, code quality analyzers([`CAxxxx` rules](code-analysis-for-managed-code-warnings.md)) are included with the [.NET SDK](/dotnet/fundamentals/productivity/code-analysis.md/#code-quality-analysis). Each code quality analyzer can be refined to apply to the parts of your codebase through configurable options. Each option is specified by adding a key-value pair to an [EditorConfig](https://editorconfig.org) file. A configuration file can be specific to a file, project, solution, or the entire repo.

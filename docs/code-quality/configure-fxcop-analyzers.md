...
This section lists the available configuration options, their allowable values, and the configurable rules for each option.
...
| `exclude_async_void_methods`<br/><br/>(`skip_async_void_methods` in package version 2.6.3 and earlier) | Whether to exclude asynchronous methods that don't return a value from the rule | `true`, `false` | `false` | [CA2007](ca2007-do-not-directly-await-task.md) |
| `exclude_single_letter_type_parameters`<br/><br/>(`allow_single_letter_type_parameters` in package version 2.6.3 and earlier) | Whether to exclude single-character [type parameters](/dotnet/csharp/programming-guide/generics/generic-type-parameters) from the rule, for example, `S` | `true`, `false` | `false` | [CA1715](ca1715-identifiers-should-have-correct-prefix.md) |

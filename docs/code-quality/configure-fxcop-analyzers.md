...
| `allow_single_letter_type_parameters` | Whether to allow single-character [type parameters](/dotnet/csharp/programming-guide/generics/generic-type-parameters), for example, `T`  | `true`, `false` | `false` | [CA1715](ca1715-identifiers-should-have-correct-prefix.md) |
...
To enable .editorconfig-based analyzer configuration for a specific project, save the .editorconfig file to the project's root directory. (Currently there is no hierarchical support for "combining" .editorconfig files that exist at different directory levels, for example, solution and project level.)

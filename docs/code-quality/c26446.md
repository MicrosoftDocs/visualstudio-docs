...
- The logic to warn on overloaded operator[] calls is more complex:
  ...
  - If the operator is marked as non-throwing (e.g. by using **noexcept**, or **throw()**, or **__declspec(nothrow)**), the call is flagged. It is assumed that if the subscript operator never throws exceptions it either doesn’t perform range checks or these checks are obscure.
  - If the operator is not marked as non-throwing it may be flagged if it comes from an STL container which also defines a conventional `at()` member function (such functions are detected by simple name matching).
  - The rule doesn’t warn on calls to standard `at()` methods. These are safe and replacing them with `gsl::at()` would not bring much value.

...
| CA1834 |[CA1834: Use StringBuilder.Append(char) for single character strings](../code-quality/ca1834.md) | <xref:System.StringBuilder> has an `Append` overload that takes a `char` as it's argument. Prefer calling the `char` overload for performance reasons. |

...
| CA5366 | [CA5366 Use XmlReader For DataSet Read XML](../code-quality/ca5366.md) | Using a <xref:System.Data.DataSet?displayProperty=nameWithType> to read XML with untrusted data may load dangerous external references, which should be restricted by using an <xref:System.Xml.XmlReader?displayProperty=nameWithType> with a secure resolver or with DTD processing disabled. |

...
- A call is made to the default (parameterless) constructor of an exception type that is, or derives from, <xref:System.ArgumentException> that have a constructor accepting a `paramName` parameter.
...
- One of the parameter's name is passed for the `message` argument of the constructor of exception type that is, or derives from, <xref:System.ArgumentException>.
...
>
> In case the <xref:System.ArgumentException>(string `message`) overload is used mistakenly with a parameter name, the fixer would provide option to use two arguments constractor with `null` message 
> 

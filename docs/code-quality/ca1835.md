...
The rule works on `ReadAsync` and `WriteAsync` invocations of any class that inherits from <xref:IO.Stream>.
...
|`ReadAsync(Byte[], Int32, Int32)`|`ReadAsync(Memory{Byte})` with `CancellationToken = default`. |
|`WriteAsync(Byte[], Int32, Int32, CancellationToken)`|`WriteAsync(ReadOnlyMemory{Byte}, CancellationToken)`|
|`WriteAsync(Byte[], Int32, Int32)`|`WriteAsync(ReadOnlyMemory{Byte})` with `CancellationToken = default`. |
...
Replace the byte-array-based method overload with a memory-based method overload.

Here are some examples of rule violations and how to fix them:

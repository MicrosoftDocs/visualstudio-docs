...
<xref:System.Text.StringBuilder.Append> and <xref:System.Text.StringBuilder.Insert> provide overloads for multiple types beyond <xref:System.String>.  When possible, prefer the strongly-typed overloads over using ToString() and the string-based overload.

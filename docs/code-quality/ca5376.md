...
Using `GetSharedAccessSignature` under <xref:Microsoft.WindowsAzure.Storage?displayProperty=fullName> to generate a Shared Access Signature (SAS) with specifying `protocols` as `HttpsOrHttp`.

...
|[CA2015: Do not define finalizers for types derived from MemoryManager<T>](../code-quality/ca2015.md) | Adding a finalizer to a type derived from <xref:System.Buffers.MemoryManger%601> may permit memory to be freed while it is still in use by a <xref:System.Span%601>. |
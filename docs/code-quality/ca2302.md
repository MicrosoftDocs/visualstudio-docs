...
public class ExampleClass
{
    public BookRecord DeserializeBookRecord(byte[] bytes)
    {
        ...
        // Ensure that formatter.Binder is always assigned a non-null value before deserializing.
...
Public Class ExampleClass
    Public Function DeserializeBookRecord(bytes As Byte()) As BookRecord
        ...
        ' Ensure that formatter.Binder is always assigned a non-null value before deserializing
        formatter.Binder = If(Binders.BookRecord, New Exception("Expected non-null binder"))

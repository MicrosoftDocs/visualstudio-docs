...
- If it is a authentication cookie and is set as secure with <xref:Microsoft.AspNetCore.Authorization.AuthorizationMiddleware?displayProperty=fullName> in `Startup.Configure`, such as in the following example:
...
public class Startup
{
    ...
    {
        services.AddAuthentication("FiverSecurityCookie")
                .AddCookie("FiverSecurityCookie", options =>
                {
                    options.LoginPath = new PathString("/Security/Login");
                    options.Cookie = new CookieBuilder()
                    {
                        // It defaults to Microsoft.AspNetCore.Http.CookieSecurePolicy.SameAsRequest.
...
The configuration above is used for ASP.NET Core 2.0+ and ASP.NET Core 1.x is set up differently with the <xref:Owin.CookieAuthenticationExtensions.UseCookieAuthentication%2A?displayProperty=fullName> API.

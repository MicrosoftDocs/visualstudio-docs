...
The violation can either be fixed manually, or, in some cases, by running the code-fixer in Visual Studio. Examples:
...
In some cases, a unit length `const string` class field for example, a code-fix is not suggested by Visual Studio (but the analyzer still fires). These instances require a manual fix.

...
This rule locates calls to `String.IndexOf` where the result is used to check for the presence/absence of a substring, and suggests using `String.Contains` instead, to improve readability.

## Rule description

When `String.IndexOf` is used to check if the result is equal to `-1` or greater or equal than `0`, the call can be safely substituted with `String.Contains` without an impact in performance.

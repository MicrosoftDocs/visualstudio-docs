---
title: "CA2249: Consider using 'String.Contains' instead of 'String.IndexOf'"
ms.date: 08/27/2020
ms.topic: reference
f1_keywords:
- CA2249
...
This rule locates calls to <xref:System.String.IndexOf> where the result is used to check for the presence/absence of a substring, and suggests using <xref:System.String.Contains> instead, to improve readability.

## Rule description

When <xref:System.String.IndexOf> is used to check if the result is equal to `-1` or greater or equal than `0`, the call can be safely substituted with <xref:System.String.Contains> without an impact on performance.

Depending on the <xref:System.String.IndexOf> overload being used, the suggested fix could get a `comparisonType` argument added:

...
It is fine to suppress violations in following cases:

1. The type is designed to be inherited. The absence of the `static` modifier suggests that the type is useful as a base type.
2. The type is used as a type argument. Static types cannot be used as type arguments.

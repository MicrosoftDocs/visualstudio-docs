...
> [!TIP]
> A code fix is available for this rule in Visual Studio. To use it, position the cursor on the array violation and press **Ctrl**+**.** (period). Choose **Use AsMemory instead of the Range-based indexer on an array** from the list of options that is presented.
>
...
It's safe to suppress a violation of this rule if creating a copy is intended. To suppress this warning, you can simply add an explicit cast.

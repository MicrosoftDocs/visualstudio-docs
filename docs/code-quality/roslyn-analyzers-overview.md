...
Code analyzers inspect code at build time, like legacy analysis if it's enabled, but also live as you type. You can [configure the scope of live code analysis](./how-to-configure-live-code-analysis-scope-for-managed-code.md) to execute for the current document only, all open documents, or the entire solution.

...
The data or rawData parameter of X509Certificate constructor is hard-coded by one of the following:
...
If the hard-coded data doesn't contain the certificate's private key. For example, the data is from `.cer` file. Hard-coding public certificate information may still create a challenge for rotating certificates as they expire or get revoked.

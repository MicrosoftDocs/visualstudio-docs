...
You can potentially suppress this warning if the <xref:system.xml.schema.xmlschema.read?displayProperty=nameWithType> is always used to process XML that comes from a trusted source and hence cannot be tampered with.

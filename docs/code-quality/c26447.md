...
This rule amends another rule, [C26440 DECLARE_NOEXCEPT](c26440.md), which tries to find functions that are good candidates to be marked as **noexcept**. In this case, the idea is that once some function is marked as **noexcept**, it must keep its contract by not invoking other code which may throw exceptions.
...
- The rule focuses only on function calls and flags targets that are not **constexpr** and can potentially throw exceptions (i.e. not marked explicitly as non-throwing by using **noexcept**, **__declspec(nothrow)**, **throw()**).

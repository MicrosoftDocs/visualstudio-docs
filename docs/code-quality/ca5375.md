...
Generating an account Shared Access Signature (SAS) with <xref:Microsoft.WindowsAzure.Storage.GetSharedAccessSignature%2A?displayProperty=fullName>.
...
It is safe to suppress this rule if you're sure that the permissions of all resources are restricted as possible.

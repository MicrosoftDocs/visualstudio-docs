...
By default, this rule only looks at externally visible types, but this is [configurable](#configurability). You can also configure additional interfaces requiring there generic pendent to be implemented.
...
You can configure the List (separated by '|') of interface names with their required generic fully qualified interface (separated by '->').

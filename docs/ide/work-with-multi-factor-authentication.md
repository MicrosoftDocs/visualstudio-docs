...
Based on your development activities and resource configuration, a new resource might trigger a new CA/MFA challenge later in the session. If that happens, your account might become stale and you'll need to re-enter your credentials to satisfy that new CA/MFA challenge.

> [!NOTE] 
> Keep your browser open through the whole process (till all CA/MFA challenges for your resources are properly satisfied) for the best experience, as closing the browser could result in losing previously built MFA state and additional authorization prompts.

...
4. Find the output path entry on the page&mdash;it differs depending on your project type:
   ...
   > [!NOTE]
   > Some projects (.NET and ASP.NET Core) will by default include framework version or runtime identifier in the build path. To change this, right-click the project node in **Solution Explorer**, select **Edit Project File**, and add the following:

...
4. Find the output path entry on the page&mdash;it differs depending on your project type:
   ...
   - **Output path** in C# and JavaScript
   - **Build output path** in Visual Basic
   - **Output directory** in Visual C++

   Type in the path to generate output to or choose **Browse** to browse to that folder instead.

...
|Auto Surround|With Visual Studio 2022 version 17.9 Preview 2 or later, you can [automatically surround a selection with matching delimiters](/visualstudio/releases/2022/release-notes-preview#auto-surround-with-quotes-or-brackets) when using double quotes(`""`), single quotes(`''`), brackets(`()`, `{}`, `[]`), or any delimiters supported by the file's language. When you add an opening delimiter (`"`, `'`, `(`) to a selection in your code file, the matching closing delimiter is automatically placed around the selection.<br /><br />You can turn auto surround on or off with the **Automatically surround selections when typing quotes or brackets** setting in the **Display** section of the **Tools** > **Options** > **Text Editor** page.|

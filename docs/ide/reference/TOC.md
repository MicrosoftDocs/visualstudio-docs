# [Visual Studio Reference](visual-studio-reference.md)
# [General User Interface Elements](general-user-interface-elements-visual-studio.md)
## [Call Hierarchy](call-hierarchy.md)
## [Choose Toolbox Items, WPF Components](choose-toolbox-items-wpf-components.md)
## [Code Snippet Picker](code-snippet-picker.md)
## [Command Window](command-window.md)
## [Convert Dialog Box](convert-dialog-box.md)
## [Error List Window](error-list-window.md)
## [File Properties, JavaScript](file-properties-javascript.md)
## [Go To Line](go-to-line.md)
## [Immediate Window](immediate-window.md)
## [Miscellaneous Files](miscellaneous-files.md)
## [Options Dialog Box](options-dialog-box-visual-studio.md)
### [Environment Options Dialog Box](environment-options-dialog-box.md)
#### [Environment &mdash; General](general-environment-options-dialog-box.md)
#### [Environment &mdash; AutoRecover](autorecover-environment-options-dialog-box.md)
#### [Environment &mdash; Documents](documents-environment-options-dialog-box.md)
#### [Environment &mdash; Extensions and Updates](extensions-and-updates-environment-options-dialog-box.md)
#### [Environment &mdash; Find and Replace](find-and-replace-environment-options-dialog-box.md)
#### [Environment &mdash; Fonts and Colors](fonts-and-colors-environment-options-dialog-box.md)
#### [Environment &mdash; Import and Export Settings](import-and-export-settings-environment-options-dialog-box.md)
#### [Environment &mdash; International Settings](international-settings-environment-options-dialog-box.md)
#### [Environment &mdash; Keyboard](keyboard-environment-options-dialog-box.md)
#### [Environment &mdash; Notifications](notifications-environment-options-dialog-box.md)
#### [Environment &mdash; Quick Launch](quick-launch-environment-options-dialog-box.md)
#### [Environment &mdash; Startup](startup-environment-options-dialog-box.md)
#### [Environment &mdash; Synchronized Settings](synchronized-settings-environment-options-dialog-box.md)
#### [Environment &mdash; Tabs and Windows](tabs-and-windows-environment-options-dialog-box.md)
#### [Environment &mdash; Task List](task-list-environment-options-dialog-box.md)
#### [Environment &mdash; Web Browser](web-browser-environment-options-dialog-box.md)
#### [Options Page, Environment Node Properties](options-page-environment-node-properties.md)
##### [Options Page, Fonts and Colors Node Properties](options-page-fonts-and-colors-node-properties.md)
### [Text Editor Options Dialog Box](text-editor-options-dialog-box.md)
#### [Text Editor &mdash; General](options-text-editor-general.md)
#### [Text Editor &mdash; File Extension](options-text-editor-file-extension.md)
#### [Text Editor &mdash; All Languages](options-text-editor-all-languages.md)
#### [Text Editor &mdash; All Languages, Tabs](options-text-editor-all-languages-tabs.md)
#### [Text Editor &mdash; Basic (Visual Basic)](options-text-editor-basic-visual-basic.md)
#### [Text Editor &mdash; C/C++, Formatting](options-text-editor-c-cpp-formatting.md)
#### [Text Editor &mdash; C/C++, Advanced](options-text-editor-c-cpp-advanced.md)
#### [Text Editor &mdash; C/C++, Experimental](options-text-editor-c-cpp-experimental.md)
#### [Text Editor &mdash; C#, Formatting](options-text-editor-csharp-formatting.md)
#### [Text Editor &mdash; C#, Advanced](options-text-editor-csharp-advanced.md)
#### [Text Editor &mdash; C#, IntelliSense](options-text-editor-csharp-intellisense.md)
#### [Text Editor &mdash; JavaScript, Formatting](options-text-editor-javascript-formatting.md)
#### [Text Editor &mdash; JavaScript, IntelliSense](options-text-editor-javascript-intellisense.md)
#### [Text Editor &mdash; XAML, Formatting](options-text-editor-xaml-formatting.md)
#### [Options Page, Text Editor Node Properties](options-page-text-editor-node-properties.md)
### [Options Page, Debugging Node Properties](options-page-debugging-node-properties.md)
### [Projects and Solutions Options Dialog Box](projects-and-solutions-options-dialog-box.md)
#### [Projects and Solutions &mdash; Build and Run](options-dialog-box-projects-and-solutions-build-and-run.md)
#### [Projects and Solutions &mdash; Web Projects](options-dialog-box-projects-and-solutions-web-projects.md)
#### [Projects and Solutions &mdash; VB Defaults](visual-basic-defaults-projects-options-dialog-box.md)
#### [Projects and Solutions &mdash; VC++ Project Settings](vcpp-project-settings-projects-and-solutions-options-dialog-box.md)
### [XAML Designer Options Dialog Box](xaml-designer.md)
## [Output Window](output-window.md)
## [Project Properties Reference](project-properties-reference.md)
### [Application Page, Project Designer (Visual Basic)](application-page-project-designer-visual-basic.md)
#### [Assembly Information Dialog Box](assembly-information-dialog-box.md)
### [Application Page, Project Designer (C#)](application-page-project-designer-csharp.md)
### [Build Events Page, Project Designer (C#)](build-events-page-project-designer-csharp.md)
#### [Pre-build Event-Post-build Event Command Line Dialog Box](pre-build-event-post-build-event-command-line-dialog-box.md)
### [Build Page, Project Designer (C#)](build-page-project-designer-csharp.md)
#### [Advanced Build Settings Dialog Box (C#)](advanced-build-settings-dialog-box-csharp.md)
### [Code Analysis, Project Designer](code-analysis-project-designer.md)
### [Compile Page, Project Designer (Visual Basic)](compile-page-project-designer-visual-basic.md)
#### [Advanced Compiler Settings Dialog Box (Visual Basic)](advanced-compiler-settings-dialog-box-visual-basic.md)
#### [Build Events Dialog Box (Visual Basic)](build-events-dialog-box-visual-basic.md)
### [Debug Page, Project Designer](debug-page-project-designer.md)
### [My Extensions Page, Project Designer (Visual Basic)](my-extensions-page-project-designer-visual-basic.md)
### [Publish Page, Project Designer](publish-page-project-designer.md)
#### [Prerequisites Dialog Box](prerequisites-dialog-box.md)
### [References Page, Project Designer (Visual Basic)](references-page-project-designer-visual-basic.md)
### [Security Page, Project Designer](security-page-project-designer.md)
#### [Advanced Security Settings Dialog Box](advanced-security-settings-dialog-box.md)
### [Services Page, Project Designer](services-page-project-designer.md)
#### [Advanced Settings for Services Dialog Box](advanced-settings-for-services-dialog-box.md)
### [Signing Page, Project Designer](signing-page-project-designer.md)
## [Property Pages, JavaScript](property-pages-javascript.md)
## [Properties Window](properties-window.md)
## [Toolbox](toolbox.md)
### [Toolbox, Components Tab](toolbox-components-tab.md)
### [Toolbox, Data Tab](toolbox-data-tab.md)
### [Toolbox, HTML Tab](toolbox-html-tab.md)
# [Devenv Command Line Switches](devenv-command-line-switches.md)
## [-? (devenv.exe)](q-devenv-exe.md)
## [-Build (devenv.exe)](build-devenv-exe.md)
## [-Clean (devenv.exe)](clean-devenv-exe.md)
## [-Command (devenv.exe)](command-devenv-exe.md)
## [-DebugExe (devenv.exe)](debugexe-devenv-exe.md)
## [-Deploy (devenv.exe)](deploy-devenv-exe.md)
## [-Diff](diff.md)
## [-Edit (devenv.exe)](edit-devenv-exe.md)
## [-LCID (devenv.exe)](lcid-devenv-exe.md)
## [-Log (devenv.exe)](log-devenv-exe.md)
## [-Out (devenv.exe)](out-devenv-exe.md)
## [-Project (devenv.exe)](project-devenv-exe.md)
## [-ProjectConfig (devenv.exe)](projectconfig-devenv-exe.md)
## [-Rebuild (devenv.exe)](rebuild-devenv-exe.md)
## [-ResetAddin (devenv.exe)](resetaddin-devenv-exe.md)
## [-InstallVSTemplates (devenv.exe)](installvstemplates-devenv-exe.md)
## [-ResetSettings (devenv.exe)](resetsettings-devenv-exe.md)
## [-ResetSkipPkgs (devenv.exe)](resetskippkgs-devenv-exe.md)
## [-Run (devenv.exe)](run-devenv-exe.md)
## [-Runexit (devenv.exe)](runexit-devenv-exe.md)
## [-SafeMode (devenv.exe)](safemode-devenv-exe.md)
## [-Setup (devenv.exe)](setup-devenv-exe.md)
## [-Updateconfiguration (devenv.exe)](updateconfiguration-devenv-exe.md)
## [-Upgrade (devenv.exe)](upgrade-devenv-exe.md)
## [-UseEnv (devenv.exe)](useenv-devenv-exe.md)
# [Visual Studio Commands](visual-studio-commands.md)
## [Visual Studio Command Aliases](visual-studio-command-aliases.md)
## [Add Existing Item Command](add-existing-item-command.md)
## [Add Existing Project Command](add-existing-project-command.md)
## [Add New Item Command](add-new-item-command.md)
## [Alias Command](alias-command.md)
## [Evaluate Statement Command](evaluate-statement-command.md)
## [Find Command](find-command.md)
## [Find in Files Command](find-in-files-command.md)
## [Go To Command](go-to-command.md)
## [Import and Export Settings Command](import-and-export-settings-command.md)
## [List Call Stack Command](list-call-stack-command.md)
## [List Disassembly Command](list-disassembly-command.md)
## [List Memory Command](list-memory-command.md)
## [List Modules Command](list-modules-command.md)
## [List Registers Command](list-registers-command.md)
## [List Source Command](list-source-command.md)
## [List Threads Command](list-threads-command.md)
## [Log Command Window Output Command](log-command-window-output-command.md)
## [New File Command](new-file-command.md)
## [Open File Command](open-file-command.md)
## [Open Project Command](open-project-command.md)
## [Open Solution Command](open-solution-command.md)
## [Print Command](print-command.md)
## [Quick Watch Command](quick-watch-command.md)
## [Replace Command](replace-command.md)
## [Replace In Files Command](replace-in-files-command.md)
## [Set Current Process](set-current-process.md)
## [Set Current Stack Frame Command](set-current-stack-frame-command.md)
## [Set Current Thread Command](set-current-thread-command.md)
## [Set Radix Command](set-radix-command.md)
## [Shell Command](shell-command.md)
## [ShowWebBrowser Command](showwebbrowser-command.md)
## [Start Command](start-command.md)
## [Symbol Path Command](symbol-path-command.md)
## [Toggle Breakpoint Command](toggle-breakpoint-command.md)
## [Watch Command](watch-command.md)
# [XML Tools in Visual Studio](../../xml-tools/xml-tools-in-visual-studio.md)
# [MSBuild](../../msbuild/msbuild.md)
# [Accessibility for People with Disabilities](accessibility-for-people-with-disabilities.md)
## [Accessibility Features of Visual Studio](accessibility-features-of-visual-studio.md)
### [How to: Set IDE Accessibility Options](how-to-set-ide-accessibility-options.md)
### [How to: Use the Keyboard Exclusively](how-to-use-the-keyboard-exclusively.md)
### [Accessibility Tips and Tricks](accessibility-tips-and-tricks.md)
## [Accessibility Products and Services from Microsoft](accessibility-products-and-services-from-microsoft.md)
## [Resources for Designing Accessible Applications](resources-for-designing-accessible-applications.md)
# [Resources for Troubleshooting Integrated Development Environment Errors](resources-for-troubleshooting-integrated-development-environment-errors.md)

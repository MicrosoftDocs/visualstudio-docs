...
**What:** Fades out unused parameters and generates a warning for unused expression values. The compiler also does a flow analysis to find any unused value assignments. Unused value assignments fade out and have a code fix to remove the redundant assignment. Unused variables with unknown values get a suggestion to use [discards](https://docs.microsoft.com/en-us/dotnet/csharp/discards). Discards are temporary, dummy variables that are intentionally unused in application code. They can reduce memory allocation and make your code easier to read.

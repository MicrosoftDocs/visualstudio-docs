...
#### Edit and Continue
Edit and Continue doesn't work in projects that are not loaded when you start debugging. The files contained in such project will be read-only and an error will be reported that the project has not been loaded if an edit is attempted.

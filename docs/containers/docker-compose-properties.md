...
The normal way that Docker provides for overriding standard Docker Compose settings is to use the *docker-compose.override.debug.yml* and *docker-compose.override.release.yml* files, but you cannot override Visual Studio-specific settings in those files. You can override Visual Studio settings by placing a file named *docker-compose.vs.debug.yml* (for **Fast** mode) or *docker-compose.vs.release.yml* (for **Regular** mode) in the same directory as your *docker-compose.yml* file. 

>[!TIP] 
>To find out the default values for any of the Visual Studio settings, look in *docker-compose.partial.debug.g.yml* or *docker-compose.partial.release.g.yml*. These files themselves are generated by Visual Studio and should not be modified.

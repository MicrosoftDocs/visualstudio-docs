...
| publishAllPorts    | "publishAllPorts": true                                 | If true, pass `-P` to `docker run` which will publish all exposed ports to a random port. However, when you specify sslPort, Visual Studio still passes `-p 5002:443` so your service should still be listening on port 5002.|

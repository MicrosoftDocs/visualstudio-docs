...
For .NET 8, additional mount points at root and for the app user that contain user secrets and certificates might also be present. And in Visual Studio 17.10 preview, the hot reload and token service proxy volumes are combined under a single mount point, `VSTools`.

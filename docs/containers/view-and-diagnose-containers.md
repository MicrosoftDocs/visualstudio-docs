...
- For the Docker Compose node, [Visual Studio 2022 version 17.7 or later](https://visualstudio.microsoft.com/downloads)
...
:::moniker range=">=vs-2022"
If you're using Docker Compose with Visual Studio 2022 version 17.7 or later, you have the option of viewing logs separately, or interleaved into a single output stream. If you select the parent node for the solution, you see interleaved logs from all the running containers. The first column on each line shows the container that produced that line of output. If you only want to see the logs for one container by itself, select that specific project's node.
...
:::moniker range=">=vs-2022"
In Visual Studio 2022 version 17.7 or later, when targeting .NET 7 or later, the Dockerfile might contain the  `USER app` command, which specifies to run the app with normal user permissions. The **Files** tab uses the those permissions as well, and so you might not be able to view some folders, if those folders are set to require elevated permissions to view.
:::moniker-end
...
:::moniker range=">=vs-2022"
Your Dockerfile can specify the `USER app` command, which means your app runs with normal user permissions, rather then elevated permissions. The Dockerfile generated by Visual Studio includes this in Visual Studio 2022 version 17.7 and later, if you're targeting .NET 7 or later. The terminal opens with the same permissions as the app uses to run. So if the Dockerfile specifies the `app` user, it runs with normal permissions; otherwise, the terminal runs with elevated permissions.
:::moniker-end

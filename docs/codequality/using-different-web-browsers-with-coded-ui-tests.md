...
3.  Highlight the extension and choose **Download**.  
 ...
 For more information about creating and using coded UI tests, see [Creating Coded UI Tests](../codequality/use-ui-automation-to-test-your-code.md#VerifyingCodeUsingCUITCreate).  
...
##  <a name="generate"></a> *I don't see the option to change BrowserWindow.CurrentBrowser in the coded UI test.*  
 You might be using a version of [!INCLUDE[vs2011_first](../codequality/includes/vs2011_first_md.md)] that does not support coded UI tests using various web browsers. To use such coded UI tests, you must use Visual Studio Enterprise.  
  
 *What else should I know?*  
 **Notes**  
  
-   ![Prerequsite](../codequality/media/prereq.png "Prereq") Apple Safari web browser is not supported.  
  
-   ![Prerequsite](../codequality/media/prereq.png "Prereq") The action of starting the web browser must be part of the coded UI test.  
...
-   ![Prerequsite](../codequality/media/prereq.png "Prereq") Automating browser specific based UI actions such as maximize, minimize and restore is not supported.  
  
 **Tips**  
  
-   ![Tip](../codequality/media/tip.png "Tip") You can configure the output to include screenshots in the coded UI logs. To do so, you need to set some configuration settings in the QTAgent32.exe.config file. By default, this file is installed in the following location:  
    ...
    -   `<add name="EqtTraceLevel" value="4" />`  
     ...
     For more information, see [Analyzing Coded UI Tests Using Coded UI Test Logs](../codequality/analyzing-coded-ui-tests-using-coded-ui-test-logs.md).  

...
| .NET Core projects (.xproj) | If you allow Visual Studio to upgrade a .xproj project that was created using Preview tooling based on Visual Studio 2015, then such projects will be changed in such a way that they will not open in prior versions of Visual Studio. |
| ASP.NET Web Application and ASP.NET Core Web Application with Application Insights enabled |For each Visual Studio user, resource information is stored in the registry per user instance.  This is used when user don’t have a project opened and wants to search Azure Application Insights data.  VS2015 uses different registry location than VS2017 and would not conflict.<br/><br/>Once user creates an ASP.NET Web Application or ASP.NET Core Web Application, then the resource is stored in the .suo file. The user can open the project in VS2015 or VS2017 and the resource information should be used for both VS2015 and VS2017 as long as VS supports projects and solutions being used across both versions. User will need to authenticate once on each product; if project was created with VS2015, when they open it up with VS2017, they'll need to authenticate on VS2017. |
...
| C#/Visual Basic Webform or Windows Form | You can open the project in Visual Studio 2012 and later. |
...
| Database Unit Test Projects	| Older Data Unit test Projects will be loaded in Visual Studio 2017 but will use the GAC’d version of dependencies. To upgrade the unit test project to use the latest dependencies right click on the project not in Solution Explorer and click Upgrade from context menu. |
| Visual Studio Lab Management | You can use Microsoft Test Manager, Visual Studio 2015, Visual Studio 2013, Visual Studio 2012, and Visual Studio 2010 SP1 to open environments that were created in any of these versions. However, for Visual Studio 2010 SP1 the version of Microsoft Test Manager must match the version of Team Foundation Server before you can create environments. |

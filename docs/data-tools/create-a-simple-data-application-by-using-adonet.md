...
1. In Solution Explorer, add two Windows forms to your project so that it has three forms, and then give them the following names:
...
1. Right-click the **Sales** data connection in **Server Explorer** and choose **Properties**. 

1. Locate the **Connection String** property, and then copy the string to the clipboard.
...
- **Add an account**
   ...
   Opens the NewCustomer form.
- **Fill or cancel orders**
 
   Opens the FillOrCancel form. 
...
> [!NOTE]
> If you decide not to use the double-click action in the designer and instead copy and paste code directly into your code files, don't forget to set the event handler to the right method. You can do so in the **Properties** window: Switch to the **Events** tab by using the lightning bolt toolbar button, and search for the **Click** handler.

...
- The **.NET desktop development** and **Data storage and processing** workloads installed in Visual Studio. To install them, open **Visual Studio Installer** and choose **Modify** or **More** > **Modify** next to the version of Visual Studio you want to modify. For more information, see [Modify Visual Studio](../install/modify-visual-studio.md).
...
- The Northwind sample database, created by following these steps:
  ...
  1. Paste the T-SQL script into the query editor, and then select **Execute** at the top of the window. The Northwind database is created.
...
The **PassingDataBetweenForms** project is created and added to **Solution Explorer**. The **Form1** designer view appears in the code editor.
...
1. On the **Choose a Database Model** page, verify that **Dataset** is specified, and then select **Next**.

1. On the **Choose Your Data Connection** page, select **New Connection**.
...
1. Select **Yes** or **No** to the popup question **Would you like to copy the file to your project and modify the connection?**

1. On the **Save the Connection String to the Application Configuration file** page, select **Yes** and then select **Next**.

8. On the **Choose your Database Objects** page, expand the **Tables** node, select the **Customers** and **Orders** tables, and then select **Finish**.
...
1. In **What data should the table load**, add a `WHERE` clause to the end of the query to return `Orders` based on the `CustomerID`. The query should look similar to the following:
...
1. Right-click in **Form2** and select **View Code** to open **Form2** in the editor.

1. Add the following code to **Form2** after the `Form2_Load` method:
...
1. Double-click the **CellDoubleClick** event.
...
1. Double-click a customer record in **Form1** to open **Form2** with that customer's orders.

## Next steps

Depending on your application requirements, you might want to do several steps after passing data between forms. Some enhancements you could make to this tutorial include:

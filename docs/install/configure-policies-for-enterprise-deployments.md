...
| `AdministratorUpdatesNotifications`| `REG_DWORD`           | 0                                                   | **Enable admins to notify users to close Visual Studio to unblock the administrator update**: If Visual Studio is open or in use, then the administrator update will fail to apply. By setting this registry key, the administrator can cause a Windows toast nofication prompt to display to the user that tells them they need to close Visual Studio and save their work so that the pending update to apply. This registry key is for the administrator user. For more information, refer to the [blog post announcement](https://devblogs.microsoft.com/visualstudio/visual-studio-17-8-now-available/#administrators-can-prompt-users-to-close-to-unblock-updates). |

...
|----------------------------------|-----------------------------|-----------------------------------------------------|----------------------------|
| `AllowStandardUserControl`       | `REG_DWORD`                 | 0  | **Allows users without administrator permissions to manually manage their Visual Studio installations**: if set to 1, users without administrator permissions can manually update or rollback an update without needing to supply an administrator password. If set to 2, users without administrator permissions can use all functionality in the Visual Studio Installer manually, including modify and install from the **Available** tab.  For security reasons, Standard Users cannot run any Visual Studio installer commands programmatically while using either the `--passive` or `--quiet` parameters, regardless of what this policy is set to. |

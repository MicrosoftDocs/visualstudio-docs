...
2. Open command prompt in administrator mode. To do this, open the Windows Start menu, type "cmd", right click on the Command Prompt search result, select **Run as administrator**. In the command prompt, change the directory to the folder where the Visual Studio bootstrapper file is.
...
```
4. Follow the installer dialogs to select the components you need for your installation. For more information, see [Install Visual Studio](install-visual-studio#step-4---choose-workloads).

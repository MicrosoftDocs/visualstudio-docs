...
| `AdministratorUpdatesEnabled` | `REG_DWORD` | 1 | If missing or is set to 0, Administrator Updates will be blocked from applying to the client computer. This is for administrative use. For more information, see the [Administrative Updates](http://TODOLink) page. | 
| `AdministratorUpdatesOptOut` | `REG_DWORD` | 1 | The absence of the registry value, or a set value of 0, means that the Visual Studio user wants to receive administrator updates to Visual Studio. This is for developer user (if they have permissions). For more information, see the [Administrative Updates](http://TODOLink) page. | 

...
| `--fix` | **Optional**: Verify the contents of a layout.  If any files are found to be corrupt or missing, they are re-downloaded. Internet access is required to fix a layout. |
...
| `--path install=<path>` | **Optional**: Equivalent to `–-installPath`. Specifically, `--installPath "C:\VS"` and `--path install="C:\VS"` are equivalent. Only one of these can be used at a time. |

...
| `--force`                                          | **Optional**: This parameter forces Visual Studio to close even if any Visual Studio process is in use. Forcing Visual Studio to close might cause loss of work, so use it with caution. If --force is included in the [configuration file for an Administrator Update](./visualstudio/install/applying-administrator-updates#methods-for-configuring-an-administrator-update), then it will only be respected if the update is run in the non-default user context; --force will be ignored if the Administrator update is run in the default system context.  |

...
| `--includeRecommended`                             | **Optional**: During an install or modify command, this parameter includes the recommended components for any workloads that are installed. It doesn't include the optional components. The workloads are specified either with `--allWorkloads` or `--add`. Please note: when passing the --includeRecommended parameter, you will enable your update setting dialog to "add recommended components" for installed workloads on update. 
see "Customize update settings" in Update Visual Studio installation to recent release | Microsoft Learn for information on changing this setting.![image](https://github.com/MicrosoftDocs/visualstudio-docs-pr/assets/81188282/fafa0dbe-c1f9-43a2-a4d6-97e5d6325212)
  |

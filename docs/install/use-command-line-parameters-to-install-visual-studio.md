...
| `--productId <id>`                                 | **Optional**: The ID of the product instance that'll be modified, and used with `--channelId`. The `productID` is something like "Microsoft.VisualStudio.Product.Enterprise". |
...
| `--channelId <id>`                                 | The ID of the channel, for example, `VisualStudio.17.Release`. See [syntax examples of `--channelId`](/visualstudio/install/command-line-parameter-examples#using---channeluri). This is required for `modify` operations, alongside either `--productID` or `--installPath`.  |

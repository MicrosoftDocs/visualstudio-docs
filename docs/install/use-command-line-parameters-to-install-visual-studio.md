...
| `--includeRecommended`                             | **Optional**: During an install or modify command, this parameter includes the recommended components for any workloads that are installed. It doesn't include the optional components. The workloads are specified either with `--allWorkloads` or `--add`. Please note that by using the `--includeRecommended` parameter, you'll enable your update setting dialog to "add recommended components" for installed workloads on update. See [Customize update settings](https://learn.microsoft.com/en-us/visualstudio/install/update-visual-studio?view=vs-2022#customize-update-settings-1) to change this setting. |

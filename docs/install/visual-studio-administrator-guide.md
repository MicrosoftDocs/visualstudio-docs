...
* Decide on the update model.
  ...
  Where do you want individual [client machines to get the product updates from](update-a-network-installation-of-visual-studio.md)? Specifically, decide whether you want the client to get updates from a [company-wide network layout](create-a-network-installation-of-visual-studio.md) or from the internet.
  ...
  It's possible to [update a network layout of Visual Studio](create-a-network-installation-of-visual-studio.md#update-or-modify-your-layout) with the latest product updates so that it can be used both as an installation point for the latest update of Visual Studio, and also to maintain installations that are already deployed to client workstations. 

  Organizations that utilize enterprise deployment tools can take advantage of the fact that Visual Studio updates are available on the Microsoft Update Catalog and Windows Server Update Services. For more information, see [Enabling administrator updates](enabling-administrator-updates.md) and [Applying administrator updates](applying-administrator-updates.md).
...
* Build an installation script that uses [command-line parameters](use-command-line-parameters-to-install-visual-studio.md) to [install Visual Studio onto a client machine from a network layout](create-a-network-installation-of-visual-studio.md#install-visual-studio-onto-a-client-machine-from-a-network-installation).
...
* [Update your network layout with the desired version of Visual Studio](create-a-network-installation-of-visual-studio.md#update-or-modify-your-layout)

* [Refresh your client machine with the latest updates](update-a-network-installation-of-visual-studio.md) to Visual Studio.

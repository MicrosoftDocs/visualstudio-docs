...
The configuration file, which is in JSON format, supports the option `installerUpdateArgs`, which is an array of strings separated by commas that specify more switches you can pass into the Visual Studio installer. If the contents of the file include an invalid field or an option that isn't supported, the update fails. Note that this JSON array format differs from command-line usage, where `--installerUpdateArgs` takes one double-quoted string of space-separated switches. For more information, see [Use command-line parameters to install Visual Studio](./use-command-line-parameters-to-install-visual-studio.md#administrator-update-command-and-command-line-parameters).

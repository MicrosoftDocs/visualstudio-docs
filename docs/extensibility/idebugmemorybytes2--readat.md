...
#### Parameters  
 `pStartContext`  
 [in] The [IDebugMemoryContext2](../extensibility/idebugmemorycontext2.md) object that specifies where to start reading bytes.  
 ...
 *`pdwUnreadable` = 20  
 ...
 In this case, because `*pdwRead + *pdwUnreadable < dwCount`, the caller must make an additional call to read the remaining 30 bytes of the original 100 requested and the [IDebugMemoryContext2](../extensibility/idebugmemorycontext2.md) object passed in the `pStartContext` parameter must be advanced by 70.  

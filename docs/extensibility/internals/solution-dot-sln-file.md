...
1. The environment iterates through the project persistence blocks. In this case, there's one project.
   ...
   Each project can persist a unique project instance ID so that it can be accessed as needed by other projects in the solution. Ideally, if the solution and projects are under source code control, the path to the project should be relative to the path to of the solution. When the solution is first loaded, the project files can't be on the user's machine. By having the project file that is stored on the server relative to the solution file, makes it relatively simpler for the project file to be found and copied to the user's machine. It then copies and loads the rest of the files needed for the project.

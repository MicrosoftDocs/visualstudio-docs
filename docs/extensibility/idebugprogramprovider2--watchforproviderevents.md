...
|`PFLAG_REASON_WATCH`|Caller wants to watch for events. If this flag is not set. then the callback event is removed and the caller no longer receives notifications.|  
 ...
 `pEventCallback`  
 [in] An [IDebugPortNotify2](../extensibility/idebugportnotify2.md) object that receives the event notifications.  
...
## Example  
 The following example shows how to implement this method for a **CDebugEngine** object that exposes the [IDebugProgramProvider2](../extensibility/idebugprogramprovider2.md) interface.  

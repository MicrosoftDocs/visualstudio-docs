...
#### Parameters  
 `dwFields`  
 [in] A combination of flags from the [BPREQI_FIELDS](../extensibility/bpreqi_fields.md) enumeration that determines which fields in the `pBPRequestInfo` parameter are to be filled out.  
  
 `pBPRequestInfo`  
 [out] Specifies the [BP_REQUEST_INFO](../extensibility/bp_request_info.md) structure to be filled in with the description of the breakpoint request.  

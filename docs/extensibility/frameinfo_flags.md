...
## Remarks  
 These flags are passed to the [EnumFrameInfo](../extensibility/idebugthread2--enumframeinfo.md) and [GetInfo](../extensibility/idebugstackframe2--getinfo.md) methods to indicate which fields are to be initialized in the [FRAMEINFO](../extensibility/frameinfo.md) structure or structures.  
  
 These flags are also used to indicate which fields of the [FRAMEINFO](../extensibility/frameinfo.md) structure are used and valid when the structure is returned. These values may be combined with a bitwise `OR`.  

...
3.  If a VSPackage is loaded, the environment calls `QueryInterface` for \<xref:Microsoft.VisualStudio.Shell.Interop.IVsPackage> interface, for the interface that the VSPackage requires.  
 ...
 Specific information relating to the use of these files can be found in [Solution (.Sln) File](../extensibility/solution--.sln--file.md) and [Solution User Options (.Suo) File](../extensibility/solution-user-options--.suo--file.md).  

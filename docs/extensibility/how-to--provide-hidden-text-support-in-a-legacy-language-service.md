...
4.  Call \<xref:Microsoft.VisualStudio.TextManager.Interop.IVsHiddenTextSession.AddHiddenRegions*> to add one or more new outline regions at a time, specifying the following information in the `reHidReg` (\<xref:Microsoft.VisualStudio.TextManager.Interop.NewHiddenRegion>) parameter:  

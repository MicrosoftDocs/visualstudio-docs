...
|**Perform stress and performance analysis by using load tests:** You can create a load test and add your unit tests to it to help isolate performance and stress issues in your application.|-   [Load testing (VSTS and TFS)](/azure/devops/load-test/)|
|**Set quality gates:** You can create quality gates to enforce that tests are run before code is checked in, to help ensure the quality of the code.|-   [Check-in policies (VSTS)](/azure/devops/tfvc/add-check-policies)|

...
Run settings files can be used to configure tests that are run from the [command line](vstest-console-options.md), in the IDE, or in a [build workflow](/vsts/pipelines/test/getting-started-with-continuous-testing?view=vsts) using Azure Test Plans or Team Foundation Server (TFS).
...
- [Visual Studio test task (Azure Test Plans)](/vsts/pipelines/tasks/test/vstest?view=vsts)

...
- Support for .NET Core/.NET Standard and performance improvements are the 2 big standouts. You will notice performance is significantly faster after first run especially when you stop and start Live Unit Testing again on a solution. We now persist data generated by Live Unit Testing and reuse it as much as possible with upto-date checks. Besides these 2 big items, the following enhancements have also been done:

  - Coverage icons in the code editor for test methods are now distinguished with the prefix of a beaker icon to help user easily differentiate product code and unit test code, especially when doing a side-by-side tab group view of the code.

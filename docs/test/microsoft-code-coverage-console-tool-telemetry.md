...
Values that might contain user data, such as file paths, the user-provided session ID (from `--session-id`), and command lines, are never sent verbatim. Instead, only the fact that an option or argument was provided is recorded as the Boolean value `true`.

If you suspect that telemetry is collecting sensitive data or that data is being handled insecurely or inappropriately, file an issue in the [microsoft/codecoverage](https://github.com/microsoft/codecoverage) GitHub repository.
...
| 18.10 | Auto-generated correlation ID (a GUID) used to correlate events from a single tool run. This value is distinct from the user-provided session ID. |
...
Because telemetry is delivered through Azure Application Insights, each event also includes standard, nonpersonal context: the operating system description and a randomly generated correlation ID used to correlate events from a single run. This correlation ID isn't the user-provided session ID. This context doesn't include the contents of your code, assemblies, or coverage data.

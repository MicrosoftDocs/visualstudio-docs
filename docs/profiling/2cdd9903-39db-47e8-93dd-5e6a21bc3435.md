...
This topic describes how to use [!INCLUDE[vsprvs](../codequality/includes/vsprvs_md.md)] Profiling Tools command-line tools to collect detailed .NET memory allocation and object lifetime data for a dynamically compiled [!INCLUDE[vstecasp](../codequality/includes/vstecasp_md.md)] Web application by using the instrumentation profiling method.  
...
2.  Start the [!INCLUDE[vstecasp](../codequality/includes/vstecasp_md.md)] Web application in the typical way.  
...
## Ending the Profiling Session  
 To end a profiling session, close the target [!INCLUDE[vstecasp](../codequality/includes/vstecasp_md.md)] Web application, stop Internet Information Services (IIS) to stop the profiled process, and then shut down the profiler. Then restart IIS.  
...
## Restoring the Application and Computer Configuration  
 When you have completed all profiling, replace the web.config file, clear the profiling environment variables, and restart the computer to restore the server and the [!INCLUDE[vstecasp](../codequality/includes/vstecasp_md.md)] application to their original states.  

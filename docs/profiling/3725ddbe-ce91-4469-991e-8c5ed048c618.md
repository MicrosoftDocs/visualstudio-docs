...
This topic describes how to use [!INCLUDE[vsprvs](../codequality/includes/vsprvs_md.md)] Profiling Tools command-line tools to attach the profiler to an ASP.NET Web application and collect performance statistics by using the sampling method.  
...
1.  Do one of the following to detach the profiler from the target application:  
    ...
    -   Close the [!INCLUDE[vstecasp](../codequality/includes/vstecasp_md.md)] worker process.  

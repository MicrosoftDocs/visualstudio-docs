...
This article describes performance insights for <xref:System.Collections.Concurrent.ConcurrentDictionary%601.Values%2A?displayProperty=nameWithType>.
...
`ConcurrentDictionary.Values` takes a lock and makes a copy. Enumerate key-value pairs for efficient access to keys or values and avoid using inside loops.

If you need to use lots of `ConcurrentDictionary.Values` method calls, for example, one per request, do not use the default constructor. The default constructor uses concurrency level of 4 * number of cores on the box, and allows for resizing when more items are added. Use a smaller number as the concurrency level instead, and then check the overall memory usage.

...
## Rule description  
 During this profiling run, the processor (or processors) was consistently busy. High CPU utilization can indicate a CPU-bound application. Instrumented profiles are usually not the most effective way to investigate CPU-usage scenarios. Sampling is generally more effective when you are profiling applications that spend many of their time executing instructions on the processor.  

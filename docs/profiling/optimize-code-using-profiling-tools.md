...
If the CPU Usage tool does not provide enough information to isolate the problem, or you would like additional insights to help improve the performance, you may want decide to use one of the other profiling tools. For example:
...
As previously described, the `GetBlogTitle` method uses a LINQ query that could perhaps be optimized. For another view of the query, use the Database tool in the Performance Profiler (**Atl+F2**). You can multi-select this tool along with CPU Usage. When you've collected a trace, select the **Query** tab in the diagnostics page.

In the Query tab for the Database trace, you can see the first row shows the longest query, 2446 ms. By examining the SELECT statement generated by LINQ, you identify the first row as the query associated with the `GetBlogTitle` method. The **Records** column shows how many records the query reads. We can use this information for later comparison.
...
Next, recheck the results in the .NET Object Allocation tool, and see that `GetBlogTitle` is only responsible for 56,000 object allocations, nearly a 95% reduction from 900,000!

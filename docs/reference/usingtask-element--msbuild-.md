...
---
# UsingTask Element (MSBuild)
Maps the task that is referenced in a [Task](../reference/task-element--msbuild-.md) element to the assembly that contains the task implementation.  
...
|`Condition`|Optional attribute.<br /><br /> The condition to evaluate. For more information, see [Conditions](../reference/msbuild-conditions.md).|  
  
### Child Elements  
  
|Element|Description|  
|-------------|-----------------|  
|[ParameterGroup](../reference/parametergroup-element.md)|The set of parameters that appear on the task that is generated by the specified `TaskFactory`.|  
|[Task](../reference/task-element--msbuild-.md)|The data that is passed to the `TaskFactory` to generate an instance of the task.|  

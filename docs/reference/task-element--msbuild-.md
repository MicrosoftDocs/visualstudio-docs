...
|Element|Description|  
|-------------|-----------------|  
|[Output](../reference/output-element--msbuild-.md)|Stores outputs from the task in the project file. There may be zero or more `Output` elements in a task.|  
...
## Example  
 The following code example creates an instance of the [Csc task](../reference/csc-task.md) class, sets six of the properties, and executes the task. After execution, the value of the `OutputAssembly` property of the object is placed into an item list named `FinalAssemblyName`.  

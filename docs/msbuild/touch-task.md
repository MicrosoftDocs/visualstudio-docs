...
|`Time`|Optional `String` parameter.<br /><br /> Specifies a time.  Defaults to the current time (`Now`). The format must be parseable by  <xref:System.DateTime.Parse%2A> method using `DateTimeFormatInfo.InvariantInfo`.|

...
|`UseUtf8Encoding`|Optional `String` parameter.<br /><br />Specifies whether to use UTF8 code page when processing the command line for executed commands. Valid values are `Always`, `Never`, or `Detect`. The default is `Detect`, which means use the UTF code page only when non-ANSI characters are detected.|

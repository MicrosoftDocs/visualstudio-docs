...
| UseCommonOutputDirectory | .NET | A boolean parameter that you can set to `true` when you want all build outputs in a solution to use the same output directory; this also disables copying of dependencies from dependent project's output directories. You still need to set the output directory to the desired common output directory for each project.|

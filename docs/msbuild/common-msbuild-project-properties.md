...
| UseCommonOutputDirectory | .NET | A boolean parameter that you can set to `true` when you want all build outputs in a solution to use the same output directory; this disables copying of dependencies from dependent projects' output directories. You still need to set the output directory to the desired common output directory for each project.|

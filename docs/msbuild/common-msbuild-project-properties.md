...
| BuildProjectReferences | A boolean value that indicates whether project references are built by MSBuild. Automatically set to `false` if you are building your project in the Visual Studio integrated development environment (IDE), `true` if otherwise. `-p:BuildProjectReferences=false` can be specified on the command line to avoid checking that referenced projects are up to date. |
...
| CscToolPath | The path of *csc.exe*, the C# compiler. |
...
| SkipInvalidConfigurations | When `true`, generate a warning on invalid platform and configuration combinations, but do not fail the build; when `false` or undefined (the default), generate an error. |

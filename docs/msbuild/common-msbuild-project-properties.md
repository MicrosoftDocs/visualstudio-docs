...
| Deterministic | .NET | A boolean value that controls whether supported tasks and tools produce deterministic outputs. For identical inputs, deterministic tasks and tools produce equivalent outputs. For compiled assemblies, this property corresponds to the `/deterministic` compiler switch. |
| DeterministicTimestamp | All | Specifies a timestamp for supported tasks and tools that produce deterministic outputs. The property accepts an RFC 3339 date and time or a Unix timestamp. A Unix timestamp is the number of seconds since January 1, 1970. Supported tasks use this timestamp instead of time-dependent values that prevent reproducible outputs. For example, archive and package tasks can use this value for each archive entry. The Unix timestamp format is compatible with the `SOURCE_DATE_EPOCH` environment variable. |

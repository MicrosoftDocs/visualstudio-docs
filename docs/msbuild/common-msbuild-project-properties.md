...
| ResolveAssemblyWarnOrErrorOnTargetArchitectureMismatch | All | A boolean value that, when set to`true`, disables warning MSB3253 and MSB3270 about mismatches between the platform architecture of a build target and its dependencies, and warning MSB3276 about resolving different versions of the same dependent assemblies. |
...
| WarningLevel | All | Specifies the warning level to pass to the compilers for warnings emitted by the compiler. This settings doesn't affect MSBuild warnings, which do not have level designations. |

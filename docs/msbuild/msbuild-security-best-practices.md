...
> [!NOTE]
> As an implementation detail, MSBuild locates and communicates with its persistent processes through named pipes and mutexes. On Windows these share a machine-wide namespace, so a process under another local account can occupy those names and block your build from creating them — an availability (denial-of-service) concern only. OS access controls still prevent another account from reading or altering the build. Prefer a machine where all local accounts are trusted, or an isolated session, container, or VM.

...
| `LauncherBasedDeployment` | Optional Boolean parameter.<br /><br /> Set this to `True` when building for .NET 3.1, .NET 5 and later versions to ensure that the application executable is renamed to the expected executable name after building the application host executable *apphost.exe*. |

...
Many tasks ultimately inherit from the <xref:Microsoft.Build.Utilities.Task> class and <xref:Microsoft.Build.Utilites.ToolTask> class. This class adds several methods to the tasks that derive from [VCToolTask](../msbuild/vctooltask-base-class.md). These methods are listed in this document.

...
This error occurs when a task returned `false` to signal that it didn't complete successfully, but didn't log an error. This indicates a failure from a task that didn't report enough information about the problem.  

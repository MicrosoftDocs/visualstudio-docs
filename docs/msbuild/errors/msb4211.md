...
This error occurs when a target in an MSBuild project file attempts to use a property that was not initialized at the point it ws used. MSBuild expects all properties to have a value before they are accessed.
...
If `OutputDir` is not defined when it is used, MSBuild raises the MSB4211 error since the property is accessed without a value.

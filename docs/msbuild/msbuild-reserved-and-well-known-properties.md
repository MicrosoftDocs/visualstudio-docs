...
| `MSBuildOverrideTasksPath` | | The location to search for `overridetasks` files, which can be used to provide custom versions of standard tasks. See [MSBuild tasks](msbuild-tasks.md#overridden-tasks). |
...
| `MSBuildUserExtensionsPath` |  | If set, this path provides an alternate location for local, per-user, custom versions of the MSBuild toolset and standard build import files. See [Customize your local build](customize-your-local-build.md#msbuildextensionspath-and-msbuilduserextensionspath).  |

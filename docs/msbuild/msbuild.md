...
To install MSBuild on a Windows system that doesn't have Visual Studio, go to Build Tools for Visual Studio 2022 on the [downloads page](https://visualstudio.microsoft.com/downloads/). This method gives you MSBuild.exe installed to the Visual Studio installation folder. Another way of getting MSBuild is to install the [.NET SDK](/dotnet/core/sdk#acquiring-the-net-sdk). MSBuild is available with the .NET SDK on macOS, Windows, or Linux. You can also use the [.NET Core command-line interface (CLI)](/dotnet/core/tools/), which uses MSBuild, to build projects that target .NET Core and .NET 5 and later. When you install MSBuild with .NET SDK, you don't get `MSBuild.exe`; instead, you use `dotnet build` to invoke MSBuild.

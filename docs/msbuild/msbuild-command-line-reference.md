...
|-outputResultsCache[:cacheFile]|-orc[:cacheFile]|Output cache file where MSBuild will write the contents of its build result caches at the end of the build. If `-isolateProjects` is set to `False`, this sets it to `True`.|

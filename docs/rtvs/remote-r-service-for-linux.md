...
1. Give the connect a name and provide it's URL, such as `https://localhost:5444` (Windows Subsystem for Linux) or `https://public-ip:5444` (Azure container). Select **Save** when complete.
...
1. Run the install script:
    ...
    For a silent automation install use `sudo ./rtvs-install -s`.
...
1. In the list of available VM images, search for and select one of the following:
    ...
    - Data Science VM: `Linux Data Science`.
...
1. Start bash on Windows and follow the earlier instructions a [Physical Ubuntu computer](#physical-ubuntu-computer) *except* for step 3, start the service using the command `rtvsd`. This is because WSL currently does not support the systemd/systemctl interfaces.
...
1. If the provisioning succeeded, you can now connect to the container. Look for the public IP address, in the `ipAddress` field, which you use with the credentials in the docker file to to connect to the container from RTVS.

...
4.  Creating the dataset, reading the XML file, and displaying it in a <xref:System.Windows.Forms.DataGridView?qualifyHint=False> control  
  
5.  Adding code to display the XML schema based on the XML file in a <xref:System.Windows.Forms.TextBox?qualifyHint=False> control  
...
-   A <xref:System.Windows.Forms.DataGridView?qualifyHint=False> control that displays the contents of the XML file as data.  
  
-   A <xref:System.Windows.Forms.TextBox?qualifyHint=False> control that displays the XML schema for the XML file.  
  
-   Two <xref:System.Windows.Forms.Button?qualifyHint=False> controls.  
  
    -   One button reads the XML file into the dataset and displays it in the <xref:System.Windows.Forms.DataGridView?qualifyHint=False> control.  
  
    -   A second button extracts the schema from the dataset, and through a <xref:System.IO.StringWriter,?qualifyHint=False> displays it in the <xref:System.Windows.Forms.TextBox?qualifyHint=False> control.  
...
2.  From the **Toolbox**, drag the following controls onto the form:  
    ...
    -   One <xref:System.Windows.Forms.DataGridView?qualifyHint=False> control  
  
    -   One <xref:System.Windows.Forms.TextBox?qualifyHint=False> control  
...
## Create the dataset thatreceives the XML data  
 In this step, you create a new dataset named `authors`. For more information about datasets, see [Dataset tools in Visual Studio](../VS_raddata/Dataset-tools-in-Visual-Studio.md).  
...
2.  From the [Toolbox, Data Tab](../VS_IDE/Toolbox--Data-Tab.md), drag a **DataSet** onto **Form1**.  
  
3.  In the **Add Dataset** dialog box, select **Untyped dataset**, and then select**OK**.  
  
     **DataSet1** is added to the component tray.  
  
4.  In the **Properties** window, set the **Name** and <xref:System.Data.DataSet.DataSetName?qualifyHint=False> properties for`AuthorsDataSet`.  
  
## Create the event handler to read the XML file into the dataset  
 The **Read XML** button reads the XML file into the dataset. It then sets properties on the <xref:System.Windows.Forms.DataGridView?qualifyHint=False> control that bind it to the dataset.  
...
## Create the event handler to display the schema in the textbox  
 The **Show Schema** button creates a <xref:System.IO.StringWriter?qualifyHint=False> object that's filled with the schema and is displayed in the <xref:System.Windows.Forms.TextBox?qualifyHint=False>control.  
...
-   Edit the data in the dataset and write it back out as XML. For more information, see <xref:System.Data.DataSet.WriteXml?qualifyHint=False>.  
  
-   Edit the data in the dataset and write it out to a database. For more information, see [Saving Data](../VS_raddata/Saving-Data.md).  

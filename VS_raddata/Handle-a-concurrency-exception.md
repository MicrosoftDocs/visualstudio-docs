...
Concurrency exceptions (<xref:System.Data.DBConcurrencyException?qualifyHint=False>) are raised when two users attempt to change the same data in a database at the same time. In this walkthrough, you create a Windows application that illustrates how to catch a <xref:System.Data.DBConcurrencyException?qualifyHint=False>, locate the row that caused the error, and learn a strategy for how to handle it.  
...
3.  Create a form with a <xref:System.Windows.Forms.DataGridView?qualifyHint=False> to display the data.  
  
4.  Fill a dataset with data from the `Customers` table in the Northwind database.  
  
5.  Use the [Visual Database Tools](assetId:///6b145922-2f00-47db-befc-bf351b4809a1) in Visual Studio to directly access the `Customers` data table and change a record.  
...
-   Access to the Northwind sample database with permission to perform updates. For more information, see [How to: Install Sample Databases](../VS_raddata/How-to--Install-Sample-Databases.md).  
...
1.  On the **Data** menu, choose **Add New Data source**.  
     ...
     The [Data Source Configuration Wizard](../VS_raddata/media/Data-Source-Configuration-Wizard.png) opens.  
...
## Create a data-bound DataGridView control  
 In this section, you create a <xref:System.Windows.Forms.DataGridView?qualifyHint=False> by dragging the **Customers** item from the **Data Sources** window onto your Windows Form.  
...
4.  Drag the table onto an empty area of your form.  
     ...
     A <xref:System.Windows.Forms.DataGridView?qualifyHint=False> control named `CustomersDataGridView` and a <xref:System.Windows.Forms.BindingNavigator?qualifyHint=False> named `CustomersBindingNavigator` are added to the form that's bound to the <xref:System.Windows.Forms.BindingSource?qualifyHint=False>.This, is in, is turn bound to the `Customers` table in the `NorthwindDataSet`.  
...
1.  Select**F5** to run the application  
     ...
     The form appears with a <xref:System.Windows.Forms.DataGridView?qualifyHint=False> control on it that's filled with data from the `Customers` table.  
...
### Addcode to handle the concurrency exception  
 ...
 In this section, you add code that  attempts to update the database.You also handle any <xref:System.Data.DBConcurrencyException?qualifyHint=False> that might get raised, as well as any other exceptions.  

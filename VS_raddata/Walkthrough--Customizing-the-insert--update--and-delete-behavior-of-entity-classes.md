...
ms.tgt_pltfrm: na
ms.topic: article
H1: Walkthrough: Customizing the insert, update, and delete behavior of entity classes
...
-   Create a Windows Form that contains a <xref:System.Windows.Forms.DataGridView?qualifyHint=False> that is bound to the Customer class.  
  
-   Implement save functionality for the form.  
  
-   Create <xref:System.Data.Linq.DataContext?qualifyHint=False> methods by adding stored procedures to the O/R Designer.  
...
-   The **InsertCustomer**, **UpdateCustomer**, and **DeleteCustomer** stored procedures for the Northwind database. For more information, see [Walkthrough: Creating Update Stored Procedures for the Northwind Customers Table](../VS_raddata/Walkthrough--Creating-Update-Stored-Procedures-for-the-Northwind-Customers-Table.md).  
...
1.  In **Server Explorer**/**Database Explorer**, locate the Customer table in the SQL Server version of the Northwind sample database. For more information, see [How to: Connect to the Northwind Database](../VS_raddata/How-to--Connect-to-the-Northwind-Database.md).  
...
2.  Drag the **Customers** node from **Server Explorer**/**Database Explorer** onto the O/R Designer surface.  
    ...
    > [!NOTE]
    >  This renaming behavior is called *pluralization*. It can be turned on or off in the [Options Dialog Box](../VS_IDE/Options-Dialog-Box--Visual-Studio-.md). For more information, see [How to: Turn pluralization on and off (O/R Designer)](../VS_raddata/How-to--Turn-pluralization-on-and-off--O-R-Designer-.md).  
...
3.  Drag all three stored procedures onto the O/R Designer.  
     ...
     The stored procedures are added to the methods pane as <xref:System.Data.Linq.DataContext?qualifyHint=False> methods. For more information, see [DataContext Methods (O/R Designer)](../VS_raddata/DataContext-Methods--O-R-Designer-.md).  
...
> [!NOTE]
>  Although it is not an issue for this particular walkthrough, it is worth noting that LINQ to SQL handles database-generated values automatically for identity (auto-increment), rowguidcol (database-generated GUID), and timestamp columns during Inserts and Updates. Database-generated values in other column types will unexpectedly result in a null value. To return the database-generated values, you should manually set <xref:System.Data.Linq.Mapping.ColumnAttribute.IsDbGenerated?qualifyHint=False> to `true` and <xref:System.Data.Linq.Mapping.ColumnAttribute.AutoSync?qualifyHint=False> to one of the following: <xref:System.Data.Linq.Mapping.AutoSync?qualifyHint=False>, <xref:System.Data.Linq.Mapping.AutoSync?qualifyHint=False>, or <xref:System.Data.Linq.Mapping.AutoSync?qualifyHint=False>.  
...
10. Press F5 and verify that the deleted record was removed from the database.  
    ...
    > [!NOTE]
    >  If your application uses SQL Server Express Edition, depending on the value of the **Copy to Output Directory** property of the database file, the changes may not appear when you press F5 in step 10. For more information, see [How to: Manage Local Data Files in Your Project](../VS_raddata/How-to--Manage-Local-Data-Files-in-Your-Project.md).  

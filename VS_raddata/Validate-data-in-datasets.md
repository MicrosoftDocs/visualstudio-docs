...
>  The data row's <xref:System.Data.DataRow.BeginEdit?qualifyHint=False> method turns off the <xref:System.Data.DataTable.RowChanging?qualifyHint=False> and <xref:System.Data.DataTable.RowChanged?qualifyHint=False> events after each individual column change. In that case, the event is not raised until the <xref:System.Data.DataRow.EndEdit?qualifyHint=False> method has been called, when the <xref:System.Data.DataTable.RowChanging?qualifyHint=False> and <xref:System.Data.DataTable.RowChanged?qualifyHint=False> events are raised just once. For more information, see [Turn off constraints while filling a dataset](../VS_raddata/Turn-off-constraints-while-filling-a-dataset.md).  
 ...
 When records are updated, the <xref:System.Data.DataTable?qualifyHint=False> object raises events that you can respond to as changes are occurring and after changes are made.  
...
|Event|Description|  
|-----------|-----------------|  
|<xref:System.Data.DataTable.ColumnChanging?qualifyHint=False>|The value in a column is being changed. The event passes the row and column to you, along with the proposed new value.|  
|<xref:System.Data.DataTable.ColumnChanged?qualifyHint=False>|The value in a column has been changed. The event passes the row and column to you, along with the proposed value.|  
|<xref:System.Data.DataTable.RowChanging?qualifyHint=False>|The changes that were made to a <xref:System.Data.DataRow?qualifyHint=False> object are about to be committed back into the dataset. If you have not called the <xref:System.Data.DataRow.BeginEdit?qualifyHint=False> method, the <xref:System.Data.DataTable.RowChanging?qualifyHint=False> event is raised for each change to a column immediately after the <xref:System.Data.DataTable.ColumnChanging?qualifyHint=False> event has been raised. If you called <xref:System.Data.DataRow.BeginEdit?qualifyHint=False> before making changes, the <xref:System.Data.DataTable.RowChanging?qualifyHint=False> event is raised only when you call the <xref:System.Data.DataRow.EndEdit?qualifyHint=False> method.<br /><br /> The event passes the row to you, along with a value indicating what type of action (change, insert, and so on) is being performed.|  
|<xref:System.Data.DataTable.RowChanged?qualifyHint=False>|A row has been changed. The event passes the row to you, along with a value indicating what type of action (change, insert, and so on) is being performed.|  
|<xref:System.Data.DataTable.RowDeleting?qualifyHint=False>|A row is being deleted. The event passes the row to you, along with a value indicating what type of action (delete) is being performed.|  
|<xref:System.Data.DataTable.RowDeleted?qualifyHint=False>|A row has been deleted. The event passes the row to you, along with a value indicating what type of action (delete) is being performed.|  
  
 The <xref:System.Data.DataTable.ColumnChanging?qualifyHint=False>, <xref:System.Data.DataTable.RowChanging?qualifyHint=False>, and <xref:System.Data.DataTable.RowDeleting?qualifyHint=False> events are raised during the update process. You can use these events to validate data or perform other types of processing. Because the update is in process during these events, you can cancel it by throwing an exception, which prevents the update from finishing.  
  
 The <xref:System.Data.DataTable.ColumnChanged?qualifyHint=False>, <xref:System.Data.DataTable.RowChanged?qualifyHint=False> and <xref:System.Data.DataTable.RowDeleted?qualifyHint=False> events are notification events that are raised when the update has finished successfully. These events are useful when you want to take further action based on a successful update.  
...
>  The **Dataset Designer** creates a partial class in which validation logic can be added to a dataset. The designer-generated dataset doesn't delete or change any code in the partial class.  
 ...
 You can validate data when the value in a data column changes by responding to the <xref:System.Data.DataTable.ColumnChanging?qualifyHint=False> event. When raised, this event passes an event argument (<xref:System.Data.DataColumnChangeEventArgs.ProposedValue?qualifyHint=False>) that contains the value that's being proposed for the current column. Based on the contents of `e.ProposedValue`, you can:  
  
-   Accept the proposed value by doing nothing.  
  
-   Reject the proposed value by setting the column error (<xref:System.Data.DataRow.SetColumnError?qualifyHint=False>) from within the column-changing event handler.  
  
-   Optionally use an <xref:System.Windows.Forms.ErrorProvider?qualifyHint=False> control to display an error message to the user. For more information, see [ErrorProvider Component](../Topic/ErrorProvider%20Component%20\(Windows%20Forms\).md).  
  
 Validation can also be performed during the <xref:System.Data.DataTable.RowChanging?qualifyHint=False> event. For more information, see [How to: Validate Data During Row Changes](../Topic/How%20to:%20Validate%20Data%20During%20Row%20Changes.md).  

...
|`PFLAG_GET_PROGRAM_NODES`|Caller is asking for a list of program nodes to be returned.|  
 ...
 `pProcess`  
 [out] A [PROVIDER_PROCESS_DATA](../extensibility/provider_process_data.md) structure that is filled in with the requested information.  
  
## Return Value  
 If successful, returns `S_OK`; otherwise, returns an error code.  
  
## Remarks  
 This method is normally called by a process to obtain a list of programs running in that process. The returned information is a list of [IDebugProgramNode2](../extensibility/idebugprogramnode2.md) objects.  

...
-   [Adding a TestContext to the test class](../VS_IDE/How-To--Create-a-Data-Driven-Unit-Test.md#BKMK_Adding_a_TestContext_to_the_test_class)  
...
2.  Add a private <xref:Microsoft.VisualStudio.TestTools.UnitTesting.TestContext?qualifyHint=False> field and a public `TestContext` property to the test class.  
  
3.  Create a unit test method and add a <xref:Microsoft.VisualStudio.TestTools.UnitTesting.DataSourceAttribute?qualifyHint=False> attribute to it.  
  
4.  Use the <xref:Microsoft.VisualStudio.TestTools.UnitTesting.TestContext.DataRow?qualifyHint=False> indexer property to retrieve the values that you use in a test.  
...
###  <a name="BKMK_Using_TestContext_DataRow_to_access_the_data"></a> Using TestContext.DataRow to access the data  
 To access the data in the `AddIntegersData` table, use the `TestContext.DataRow` indexer. `DataRow` is a <xref:System.Data.DataRow?qualifyHint=False> object, so we retrieve column values by index or column names. Because the values are returned as objects, we need to convert them to the appropriate type:  
...
## See Also  
 <xref:Microsoft.VisualStudio.TestTools.UnitTesting.DataSourceAttribute?qualifyHint=True>   
 <xref:Microsoft.VisualStudio.TestTools.UnitTesting.TestContext?qualifyHint=True>   
 <xref:Microsoft.VisualStudio.TestTools.UnitTesting.TestContext.DataRow?qualifyHint=True>   
 <xref:Microsoft.VisualStudio.TestTools.UnitTesting.Assert?qualifyHint=True>   

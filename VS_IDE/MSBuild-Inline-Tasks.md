...
MSBuild tasks are typically created by compiling a class that implements the <xref:Microsoft.Build.Framework.ITask?qualifyHint=False> interface. For more information, see [Tasks](../VS_IDE/MSBuild-Tasks.md).  
...
## The Structure of an Inline Task  
 An inline task is contained by a [UsingTask](../VS_IDE/UsingTask-Element--MSBuild-.md) element. The inline task and the `UsingTask` element that contains it are typically included in a .targets file and imported into other project files as required. Here is a basic inline task. Notice that it does nothing.  
...
-   If the value of `Type` is `Class`, then the `Code` element contains code for a class that derives from the <xref:Microsoft.Build.Framework.ITask?qualifyHint=False> interface.  
  
-   If the value of `Type` is `Method`, then the code defines an override of the `Execute` method of the <xref:Microsoft.Build.Framework.ITask?qualifyHint=False> interface.  
...
> [!NOTE]
>  When defining the task class in the source file, the class name must agree with the `TaskName` attribute of the corresponding [UsingTask](../VS_IDE/UsingTask-Element--MSBuild-.md) element.  

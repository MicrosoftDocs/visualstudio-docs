...
|Element|Description|  
|-------------|-----------------|  
|[Output](../VS_IDE/Output-Element--MSBuild-.md)|Stores outputs from the task in the project file. There may be zero or more `Output` elements in a task.|  
...
## Remarks  
 ...
 If there are any [OnError](../VS_IDE/OnError-Element--MSBuild-.md) elements in the parent `Target` element of a task, they will still be evaluated if the task fails and `ContinueOnError` has a value of `false`. For more information on tasks, see [Tasks](../VS_IDE/MSBuild-Tasks.md).  
  
## Example  
 The following code example creates an instance of the [Csc task](../VS_IDE/Csc-Task.md) class, sets six of the properties, and executes the task. After execution, the value of the `OutputAssembly` property of the object is placed into an item list named `FinalAssemblyName`.  

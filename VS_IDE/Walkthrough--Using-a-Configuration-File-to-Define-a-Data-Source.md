...
-   Accessing the data sources using the <xref:Microsoft.VisualStudio.TestTools.UnitTesting.DataSourceAttribute?qualifyHint=False> class.  
...
-   `name` defines the identity used by the <xref:Microsoft.VisualStudio.TestTools.UnitTesting.DataSourceAttribute?qualifyHint=False> to specify which data source to use.  

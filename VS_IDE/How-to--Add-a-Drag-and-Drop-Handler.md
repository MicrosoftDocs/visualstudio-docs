...
-   <xref:Microsoft.VisualStudio.Modeling.Diagrams.ShapeElement.OnDragDrop?qualifyHint=False> – This method is called if the user releases the mouse button while the mouse pointer rests over this shape or diagram, if `OnDragOver(DiagramDragEventArgs e)` previously set `e.Effect` to a value other than `None`.  
...
-   <xref:Microsoft.VisualStudio.Modeling.Diagrams.ShapeElement.OnDoubleClick?qualifyHint=False> – This method is called when the user double-clicks the shape or diagram.  
  
     For more information, see [How to: Intercept a Click on a Shape or Decorator](../VS_IDE/How-to--Intercept-a-Click-on-a-Shape-or-Decorator.md).  
...
1.  Add to your **Dsl** and **DslPackage** projects the **MefExtension** files that are described in [Extend your DSL by using MEF](../VS_IDE/Extend-your-DSL-by-using-MEF.md).  
...
-   <xref:System.Windows.Forms.IDataObject?qualifyHint=False>  `Data` – This property carries serialized versions of the source objects, usually in more than one format. Its most useful functions are:  

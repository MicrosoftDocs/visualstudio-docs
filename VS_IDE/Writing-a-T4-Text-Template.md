...
-   **Control blocks** - program code that inserts variable values into the text, and controls conditional or repeated parts of the text.  
 ...
 To try the examples in this topic, copy them into a template file as described in [Design-Time Code Generation by using T4 Text Templates](../VS_IDE/Design-Time-Code-Generation-by-using-T4-Text-Templates.md). After editing the template file, save it, and then inspect the output **.txt** file.  
...
```  
 ...
 For more information about directives, see [T4 Text Template Directives](../VS_IDE/T4-Text-Template-Directives.md).  
...
```  
 ...
 For more information about control blocks, see [Text Template Control Blocks](../VS_IDE/Text-Template-Control-Blocks.md).  
...
```  
 ...
 The assembly directive has no effect in a [preprocessed text template](../VS_IDE/Run-Time-Text-Generation-with-T4-Text-Templates.md).  
  
 For more information, see [T4 Assembly Directive](../VS_IDE/T4-Assembly-Directive.md).  
...
```  
 ...
 For more information, see [T4 Import Directive](../VS_IDE/T4-Import-Directive.md).  
...
###  <a name="Include"></a> Including code and text  
 ...
 For more information, see [T4 Include Directive](../VS_IDE/T4-Include-Directive.md).  
...
### Utility methods  
 ...
 For more information, see [Text Template Utility Methods](../VS_IDE/Text-Template-Utility-Methods.md).  
 ...
 **Use a UML model**. You can generate code from a UML model. This has the advantage that the model can be edited as a diagram in a familiar notation. Also, you do not have to design the diagram. For more information, see [Generate files from a UML model](../VS_IDE/Generate-files-from-a-UML-model.md).  
  
### Relative file paths in design-time templates  
 In a [design-time text template](../VS_IDE/Design-Time-Code-Generation-by-using-T4-Text-Templates.md), if you want to reference a file in a location relative to the text template, use `this.Host.ResolvePath()`. You must also set `hostspecific="true"` in the `template` directive:  
...
### Design-time Text Templates run in a separate AppDomain  
 You should be aware that a [design-time text template](../VS_IDE/Design-Time-Code-Generation-by-using-T4-Text-Templates.md) runs in an AppDomain that is separate from the main application. In most cases this is not important, but you might discover restrictions in certain complex cases. For example, if you want to pass data in or out of the template from a separate service, then the service must provide a serializable API.  
  
 (This isn’t true of a [run-time text template](../VS_IDE/Run-Time-Text-Generation-with-T4-Text-Templates.md), which provides code that is compiled along with the rest of your code.)  
...
|Task|Topic|  
|----------|-----------|  
|Writing a template.|[Guidelines for Writing T4 Text Templates](../VS_IDE/Guidelines-for-Writing-T4-Text-Templates.md)|  
|Generate text by using program code.|[Text Template Structure](../VS_IDE/Writing-a-T4-Text-Template.md)|  

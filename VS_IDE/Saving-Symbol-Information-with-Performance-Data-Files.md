...
-   To serialize symbols in the Visual Studio IDE, point to **Tools** on the menu bar, and then click **Options**. In the **Options** window, select **Performance Tools**, and then select the **Automatically serialize symbol information** check box.  
...
-   Make sure that you run vsperfreport with the /PACKSYMBOLS flag or, in the Visual Studio IDE, that you have the serialize symbol information option selected in the general performance explorer options.  
...
-   Make sure that you have set the path of your Windows symbol cache. Do one of the following to set the symbol cache path:  
    ...
    -   Set the Debugger->Symbols option in the Visual Studio IDE to the correct path.  
  
    -   Add the -symbolpath option to the VSPerfReport command line to include your symbols.  
  
-   If you do not see any symbols in ASP.NET, make sure that you have the symbol server set up correctly for the ASP server.  

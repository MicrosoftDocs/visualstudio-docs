...
-   `BeforeTargets` and `AfterTargets`. These `Target` attributes specify that this target should run before or after the specified targets (MSBuild 4.0).  
 ...
 Targets may have a `Condition` attribute. If the specified condition evaluates to `false`, the target isn't executed and has no effect on the build. For more information about conditions, see [Conditions](../VS_IDE/MSBuild-Conditions.md).  
  
## Initial Targets  
 The `InitialTargets` attribute of the [Project](../VS_IDE/Project-Element--MSBuild-.md) element specifies targets that will run first, even if targets are specified on the command line or in the `DefaultTargets` attribute. Initial targets are typically used for error checking.  
...
```  
 ...
 For more information, see [How to: Specify Which Target to Build First](../VS_IDE/How-to--Specify-Which-Target-to-Build-First.md).  
  
## Default Targets  
 The `DefaultTargets` attribute of the [Project](../VS_IDE/Project-Element--MSBuild-.md) element specifies which target or targets are built if a target isn't specified explicitly in a command line.  

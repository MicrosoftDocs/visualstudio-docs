...
1.  In Visual Studio, set the solution configuration to **Release** and choose the deployment target.  
     ...
     ![Select Release and Local Machine](../VS_IDE/media/CPUUSE_SelectReleaseLocalMachine.png "CPUUSE_SelectReleaseLocalMachine")  
...
3.  Choose **CPU Usage** and then choose **Start**.  
     ...
     ![Choose CPU Usage](../VS_IDE/media/CPUUSE_LIB_ChooseCpuUsage.png "CPUUSE_LIB_ChooseCpuUsage")  
...
5.  After the second output line appears, choose **Stop Collection** in the Performance and Diagnostic hub.  
 ...
 ![Stop CpuUsage data collection](../VS_IDE/media/CPU_USE_WT_StopCollection.png "CPU_USE_WT_StopCollection")  
  
 The CPU Usage tool analyzes the data and displays the report.  
  
 ![CpuUsage report](../VS_IDE/media/CPU_USE_WT_Report.png "CPU_USE_WT_Report")  
...
####  <a name="BKMK_Call_tree_structure"></a> Call tree structure  
 ![GetMaxNumberButton&#95;Click call tree](../VS_IDE/media/CPU_USE_WT_GetMaxNumberCallTree_annotated.png "CPU_USE_WT_GetMaxNumberCallTree_annotated")  
  
|||  
|-|-|  
|![Step 1](../VS_IDE/media/ProcGuid_1.png "ProcGuid_1")|The top-level node in CPU Usage call trees is a pseudo-node|  
|![Step 2](../VS_IDE/media/ProcGuid_2.png "ProcGuid_2")|In most apps, when the **Show External Code** option is disabled, the second-level node is an **[External Code]** node that contains the system and framework code that starts and stops the app, draws the UI, controls thread scheduling, and provides other low-level services to the app.|  
|![Step 3](../VS_IDE/media/ProcGuid_3.png "ProcGuid_3")|The children of the second-level node are the user-code methods and asynchronous routines that are called or created by the second-level system and framework code.|  
|![Step 4](../VS_IDE/media/ProcGuid_4.png "ProcGuid_4")|Child nodes of a method contain data only for the calls of the parent method. When **Show External Code** is disabled, app methods can also contain an **[External Code]** node.|  
...
####  <a name="BKMK_External_Code"></a> External Code  
 ...
 ![Choose Filter View, then Show External Code](../VS_IDE/media/CPU_USE_WT_FilterView.png "CPU_USE_WT_FilterView")  
 ...
 ![Nested external code in the call tree](../VS_IDE/media/CPU_USE_WT_ShowExternalCodeTooWide.png "CPU_USE_WT_ShowExternalCodeTooWide")  
 ...
 ![Search for nested external code](../VS_IDE/media/CPU_USE_WT_ShowExternalCodeTooWide_Found.png "CPU_USE_WT_ShowExternalCodeTooWide_Found")  
  
###  <a name="BKMK_Call_tree_data_columns"></a> Call tree data columns  
  
|||  
|-|-|  
|**Total CPU (%)**|![Total % data equation](../VS_IDE/media/CPU_USE_WT_TotalPercentEquation.png "CPU_USE_WT_TotalPercentEquation")<br /><br /> The percentage of the app's CPU activity in the selected time range that was used by calls to the function and the functions called by the function. Note that this is different from the **CPU Utilization** timeline graph, which compares the total activity of the app in a time range to the total available CPU capacity.|  
|**Self CPU (%)**|![Self % equation](../VS_IDE/media/CPU_USE_WT_SelflPercentEquation.png "CPU_USE_WT_SelflPercentEquation")<br /><br /> The percentage of the app's CPU activity in the selected time range that was used by the calls to the function, excluding the activity of functions called by the function.|  
...
###  <a name="BKMK_Asynchronous_functions_in_the_CPU_Usage_call_tree"></a> Asynchronous functions in the CPU Usage call tree  
 ...
 ![GetMaxNumberAsyncButton&#95;Click report selection](../VS_IDE/media/CPU_USE_WT_GetMaxNumberAsync_Selected.png "CPU_USE_WT_GetMaxNumberAsync_Selected")  
 ...
 ![Expanded GetMaxNumberAsyncButton&#95;Click call tree](../VS_IDE/media/CPU_USE_WT_GetMaxNumberAsync_ExpandedCallTree.png "CPU_USE_WT_GetMaxNumberAsync_ExpandedCallTree")  

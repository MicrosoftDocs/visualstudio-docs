...
>  Adding tier interaction data to a profiling run requires specific procedures with the command line profiling tools. See [Collecting tier interaction data](../VS_IDE/Adding-tier-interaction-data-from-the-command-line.md).  
 ...
 To collect performance data from a .NET Framework service, you must use the [VSPerfCLREnv.cmd](../VS_IDE/VSPerfCLREnv.md) tool to initialize the appropriate environment variables. You must then restart the computer that hosts the service and configure that computer for profiling. You then attach the profiler to the service process. When the profiler is attached to the service, you can pause and resume data collection.  
...
6.  Start the profiler. Type:  
    ...
    |[/crosssession](../VS_IDE/CrossSession.md)|Enables profiling of processes in other sessions. This option is required if the service is running in a different session. The session id is listed in the Session ID column on the the Processes tab of Windows Task Manager. **/CS** can be specified as an abbreviation for **/crosssession**.|  
...
8.  Attach the profiler to the service. Type:  
     ...
     **VSPerfCmd**  [/attach](../VS_IDE/Attach.md) **:** {`PID`&#124;`ProcName`} [`Sample Event`] [[/targetclr](../VS_IDE/TargetCLR.md)**:**`Version`]  

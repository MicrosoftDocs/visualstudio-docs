...
*XML documentation comments* are JavaScript comments that you add to a script to provide information about code elements such as functions, fields, and variables. In Visual Studio, these text descriptions are displayed with IntelliSense when you reference the script function.  
 ...
 This topic provides a basic tutorial on using XML documentation comments. For information about using other elements, such as [<var\>](../VS_IDE/-var---JavaScript-.md) and [<value\>](../VS_IDE/-value---JavaScript-.md), and for additional code examples, see [XML Documentation Comments](../VS_IDE/XML-Documentation-Comments--JavaScript-.md). For information about providing IntelliSense information for an asynchronous callback such as a `Promise`, see [<returns\>](../VS_IDE/-returns---JavaScript-.md).  
...
-   In the function, add [<summary\>](../VS_IDE/-summary---JavaScript-.md), [<param\>](../VS_IDE/-param---JavaScript-.md), and [<returns\>](../VS_IDE/-returns---JavaScript-.md) elements, and precede each element with three slash marks (///).  
...
-   In a constructor function or object definition, add a [<field\>](../VS_IDE/-field---JavaScript-.md) element preceded by three slash marks (///).  
  
     The following example shows the use of the `<field>` element in a constructor function. For additional examples, see [<field\>](../VS_IDE/-field---JavaScript-.md).  
...
1.  In the function, add a [<signature\>](../VS_IDE/-signature---JavaScript-.md) element for each overload. In these elements, add other elements, such as `<summary>`, `<param>`, and `<returns>`, preceding each element with three slash marks (///).  
...
1.  Create an XML file that has documentation comments in the OpenAjax MessageBundle format.  
    ...
    > [!IMPORTANT]
    >  MessageBundle is the recommended format. This format is not supported in Microsoft Ajax or in .winmd files. For information about using the alternative `VSDoc` format, see [<loc\>](../VS_IDE/-loc---JavaScript-.md).  
...
2.  In your .js file, add the following code. The `<loc>` element must be declared before any script, and follows the same usage rules as the `<reference>` element. For more information, see [JavaScript IntelliSense](../VS_IDE/JavaScript-IntelliSense.md) and [<loc\>](../VS_IDE/-loc---JavaScript-.md).  

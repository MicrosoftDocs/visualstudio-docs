...
## Cause  
 A transparent method requires a <xref:System.Security.Permissions.SecurityAction?qualifyHint=False> or other security demand.  
...
## Example  
 In the following example, the rule fires on the method because the method is transparent and is marked with a LinkDemand <xref:System.Security.PermissionSet?qualifyHint=False> that contains an <xref:System.Security.Permissions.SecurityAction?qualifyHint=False>.  

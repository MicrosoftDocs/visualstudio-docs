...
|`Condition`|Optional attribute.<br /><br /> The condition to be evaluated. If the condition evaluates to `false`, the target will not execute the body of the target or any targets that are set in the `DependsOnTargets` attribute. For more information about conditions, see [Conditions](../VS_IDE/MSBuild-Conditions.md).|  
|`Inputs`|Optional attribute.<br /><br /> The files that form inputs into this target. Multiple files are separated by semicolons. The timestamps of the files will be compared with the timestamps of files in `Outputs` to determine whether the `Target` is up to date. For more information, see [Incremental Builds](../VS_IDE/Incremental-Builds.md), [How to: Build Incrementally](../VS_IDE/How-to--Build-Incrementally.md), and [Transforms](../VS_IDE/MSBuild-Transforms.md).|  
|`Outputs`|Optional attribute.<br /><br /> The files that form outputs into this target. Multiple files are separated by semicolons. The timestamps of the files will be compared with the timestamps of files in `Inputs` to determine whether the `Target` is up to date. For more information, see [Incremental Builds](../VS_IDE/Incremental-Builds.md), [How to: Build Incrementally](../VS_IDE/How-to--Build-Incrementally.md), and [Transforms](../VS_IDE/MSBuild-Transforms.md).|  
...
|Element|Description|  
|-------------|-----------------|  
|[Task](../VS_IDE/Task-Element--MSBuild-.md)|Creates and executes an instance of an MSBuild task. There may be zero or more tasks in a target.|  
|[PropertyGroup](../VS_IDE/PropertyGroup-Element--MSBuild-.md)|Contains a set of user-defined `Property` elements. Starting in the .NET Framework 3.5, a `Target` element may contain `PropertyGroup` elements.|  
|[ItemGroup](../VS_IDE/ItemGroup-Element--MSBuild-.md)|Contains a set of user-defined `Item` elements. Starting in the .NET Framework 3.5, a `Target` element may contain `ItemGroup` elements. For more information, see [Items](../VS_IDE/MSBuild-Items.md).|  
|[OnError](../VS_IDE/OnError-Element--MSBuild-.md)|Causes one or more targets to execute if the `ContinueOnError` attribute is ErrorAndStop (or `false`) for a failed task. There may be zero or more `OnError` elements in a target. If `OnError` elements are present, they must be the last elements in the `Target` element.<br /><br /> For information about the `ContinueOnError` attribute, see [Task Element (MSBuild)](../VS_IDE/Task-Element--MSBuild-.md).|  

...
|RichClientExperienceOptions|Get/Set (Enum)|Enables rich client visual experience with values in <xref:EnvDTE100.vsRichClientExperienceOptions?qualifyHint=False>.|  
...
|InitializeOpenFileFromCurrentDocument|Get/Set (Boolean)|Determines whether the <xref:EnvDTE.DTEClass.OpenFile?qualifyHint=False> command seeds the directory and file name from the last active document, or from the last place you opened a file.|  
...
|DocumentDockPreference|Get/Set (Enum)|<xref:EnvDTE100.vsDocumentDockPreferenceOptions?qualifyHint=False>. Position in the tab well in which to insert opened document.|  
...
|Property Item Name|Value|Description|  
|------------------------|-----------|-----------------|  
|OnStartUp|Get/Set (Enum)|Action to take at startup, from <xref:EnvDTE.vsStartUp?qualifyHint=False>, with values 0 through 5:<br /><br /> -   0: Open Home Page<br />-   1: Load last loaded solution<br />-   2: Show **Open Project** dialog box<br />-   3: Show **New Project** dialog box<br />-   4: Show empty environment<br />-   5: Show Start Page|  
...
|CommentTokens|SafeArray|Returns a SafeArray of comment token values. Each has the fields, `Name` (string) and `Priority` (<xref:EnvDTE.vsTaskPriority?qualifyHint=False>, High, Medium, or Low).|  
...
|ViewSourceIn|Get/Set (Enum)|<xref:EnvDTE.vsBrowserViewSource?qualifyHint=False> (Source, Design, External).|  

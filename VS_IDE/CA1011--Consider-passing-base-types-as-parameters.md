...
## Example  
 The following example shows a method, `ManipulateFileStream`, that can be used only with a <xref:System.IO.FileStream?qualifyHint=False> object, which violates this rule. A second method, `ManipulateAnyStream`, satisfies the rule by replacing the <xref:System.IO.FileStream?qualifyHint=False> parameter by using a <xref:System.IO.Stream?qualifyHint=False>.  

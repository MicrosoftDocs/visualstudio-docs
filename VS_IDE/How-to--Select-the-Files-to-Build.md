...
## Specifying Inputs  
 Items represent the inputs for a build. For more information on items, see [Items](../VS_IDE/MSBuild-Items.md).  
...
## Specifying Inputs with Wildcards  
 You can also use wildcards to recursively include all files or only specific files from subdirectories as inputs for a build. For more information about wildcards, see [Items](../VS_IDE/MSBuild-Items.md)  
...
> [!NOTE]
>  You must use wildcards with items to specifiy the inputs for a build; you cannot specify the inputs using the `Sources` attribute in MSBuild tasks such as [Csc](../VS_IDE/Csc-Task.md) or [Vbc](../VS_IDE/Vbc-Task.md). The following example is not valid in a project file:  
>   
>  `<CSC Sources="*.cs">...</CSC>`  

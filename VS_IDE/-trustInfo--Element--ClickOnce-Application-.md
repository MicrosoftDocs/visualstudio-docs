...
-   `Unrestricted`  
    ...
    ```  
     ...
     In this example, the declaration for <xref:System.Security.Permissions.EnvironmentPermission?qualifyHint=False> restricts the application to reading only the environment variable USERNAME, whereas the declaration for <xref:System.Security.Permissions.FileDialogPermission?qualifyHint=False> gives the application unrestricted use of all <xref:System.Windows.Forms.FileDialog?qualifyHint=False> classes.  
...
## Remarks  
 ...
 For more information about Permission Elevation, see [Securing ClickOnce Applications](../VS_IDE/Securing-ClickOnce-Applications.md). For more information about policy deployment, see [Trusted Application Deployment Overview](../VS_IDE/Trusted-Application-Deployment-Overview.md).  
